...
'/grid/gillis/data/hover/work/cmxci.paired/Rattus_norvegicus'
'/grid/gillis/data/hover/work/cmxci.single/Rattus_norvegicus'
...
annot_rat_vcfs = wasp_filt_vcf

for(j in 1:length(annot_rat_vcfs)){
  test = annot_rat_vcfs[[j]]
  ...
  for( i in 1:dim(test)[1]){
    ...
    if(test_gene_start <= x_gene_ends[gene_index]){
      ...
      gene_annots[i] = rat_x_meta$gene[gene_index]
      gene_biotype[i] = rat_x_meta$gene_biotype[gene_index]
    }
  }
  
  test$gene = gene_annots
  test$gene_biotype = gene_biotype
  annot_rat_vcfs[[j]] = test
}
...
```{r}
save(annot_rat_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/all_snp_vcf_3_15_23.Rdata')
...
captured_genes = c()
for( i in 1:length(annot_rat_vcfs)){
  suppressWarnings(if(is.null(dim(annot_rat_vcfs[[i]]))){next})
  captured_genes = c(captured_genes, annot_rat_vcfs[[i]]$gene)
}
...
for(i in 101:200){
  ...
  gene_ref_exp = c()
  for(j in 1:length(annot_rat_vcfs)){
    suppressWarnings(if(is.null(dim(annot_rat_vcfs[[j]]))){next})
    gene_index = annot_rat_vcfs[[j]]$gene == current_gene
    ref_counts = annot_rat_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_rat_vcfs[[j]]$alt_counts[gene_index]
...
for( i in 1:length(annot_rat_vcfs)){
  suppressWarnings(if(is.null(dim(annot_rat_vcfs[[i]]))){next})
  captured_SNPs = c(captured_SNPs, annot_rat_vcfs[[i]]$Pos)
}
...
for(i in 1:length(captured_SNPs)){
  ...
  for(j in 1:length(annot_rat_vcfs)){
    suppressWarnings(if(is.null(dim(annot_rat_vcfs[[j]]))){next})
    gene_index = annot_rat_vcfs[[j]]$Pos == current_SNP
    ref_counts = annot_rat_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_rat_vcfs[[j]]$alt_counts[gene_index]
    total_counts = ref_counts + alt_counts
...
```{r}
hist(snp_stat_df$mean_ref_ratio, breaks = seq(0,1,.01), xlab = 'Mean reference ratio per SNP', main = 'Rattus norvegicus')
...
plot(snp_stat_df$mean_ref_ratio,snp_stat_df$num_samples_present, cex = .25, xlab = 'Mean reference ratio', 
     ylab = '# of samples SNP is detected', main = 'Rattus Norvegicus Het SNPs')
abline(v = .5, col = 'red')
...
save(snp_stat_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/detected_het_snps_df_3_15_23.Rdata')

rat_snp_stat_df = snp_stat_df
save(rat_snp_stat_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/rat_snp_stat_df.Rdata')
```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/detected_het_snps_df_3_15_23.Rdata')
...
#ggsave(plot = p3, filename = 'rat_x_ref_snp_filtering_plots.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/ref_snp_filtering/', device = 'pdf',
...
num_tot_vec = vector(mode = 'numeric', length = length(annot_rat_vcfs) )
num_gene_vec = vector(mode = 'numeric', length = length(annot_rat_vcfs) )
all_intergene_ratio = c()
all_gene_ratio = c()

for(i in 1:length(annot_rat_vcfs)){
  
  if(is.null(dim(annot_rat_vcfs[[i]]))){next}
  data = annot_rat_vcfs[[i]]
...
```{r}
filt_rat_vcfs = annot_rat_vcfs

for(i in 1:length(filt_rat_vcfs)){
  test = filt_rat_vcfs[[i]]
  
  suppressWarnings(if(is.null(dim(test))){next})
  
  #Get rid of SNPs in nonannotated genes
  test = test[test$gene != "", ]
  suppressWarnings(if(dim(test)[1] == 0){filt_rat_vcfs[[i]] = NA; next})
...
```{r}
save(filt_rat_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/max_snp_vcf_3_15_23.Rdata')


```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/max_snp_vcf_3_15_23.Rdata')
...
```{r}
num_filt_genes = vector(mode = 'numeric', length = length(filt_rat_vcfs))
for(i in 1:length(filt_rat_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_rat_vcfs[[i]]))){num_filt_genes[i] = 0; next})
  num_filt_genes[i] = sum(filt_rat_vcfs[[i]]$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter])
...
```{r}
num_total_genes = vector(mode = 'numeric', length = length(annot_rat_vcfs))
for(i in 1:length(annot_rat_vcfs)){
  
  suppressWarnings(if(is.null(dim(annot_rat_vcfs[[i]]))){num_total_genes[i] = 0; next})
  num_total_genes[i] = sum(annot_rat_vcfs[[i]]$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter])
...
for(i in 1:length(index_num_samps)){
  ...
  data = filt_rat_vcfs[[index_num_samps[i]]]
...
```{r}
Ns = vector(mode='numeric', length = length(filt_rat_vcfs))

for(i in 1:length(filt_rat_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_rat_vcfs[[i]]))){Ns[i] = 0}
  else{Ns[i] = dim(filt_rat_vcfs[[i]])[1]})
...
for(i in 1: length(filt_rat_vcfs)){
  ...
  total_exp = filt_rat_vcfs[[i]]$ref_counts + filt_rat_vcfs[[i]]$alt_counts
  ref_ratios = filt_rat_vcfs[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits = mclapply(1:length(filt_rat_vcfs), function(i) est_skew_func(filt_rat_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()


Sys.time()
folded_norm_fits_bad_snps = mclapply(1:length(filt_rat_vcfs), function(i) est_skew_func(filt_rat_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df, filter_snps = F), mc.cores = 20)
Sys.time()
...
hist(est_skew, main = 'rat skew estimates filtering out bad SNPs', breaks=64)
...
num_good_snps = vector(length = length(filt_rat_vcfs), mode = 'numeric')
total_snps = vector(length = length(filt_rat_vcfs), mode = 'numeric')

for(i in 1:length(filt_rat_vcfs)){
  
  data = filt_rat_vcfs[[i]]
...
all_ref_ratios = c()
all_snp_pos = c()
rat_het_index = which(est_skew >= .6)
for(i in 1:length(rat_het_index)){

  data = annot_rat_vcfs[[rat_het_index[i]]]
  if(is.null(dim(data))){next}
...
p1 = ggplot(binned_chrom_df, aes(x = bin, y = mean_bin_ref_ratio, size = num_snps)) + geom_point(alpha = .5) + geom_hline(yintercept = .6, color = 'red') +
...
#And excluding 1MB bins with a mean reference ratio below .6

bad_rat_bins = which(mean_bin_ref_ratio<= .6)
bad_rat_bins


binned_chrom_df$keep_label = rep('Kept 1MB bin', length = nrow(binned_chrom_df))
binned_chrom_df$keep_label[(binned_chrom_df$bin/1e6) %in% (bad_rat_bins - 1)] = 'Excluded 1MB bin'
p1 = ggplot(binned_chrom_df, aes(x = bin, y = mean_bin_ref_ratio, size = num_snps, color = keep_label)) + geom_point(alpha = .5) + geom_hline(yintercept = .6, color = 'red') +
  ylab('Mean folded SNP ratio per bin') + xlab('X chromosome position (1MB bins)') + ggtitle('rat All SNPs')+ ylim(.5, 1) +
  scale_color_manual(values = c('Kept 1MB bin' = 'black', 'Excluded 1MB bin' = 'red'))  
ggMarginal(p1, margins = 'y', type = 'histogram', yparams = list(binwidth = .005))
#ggsave(filename = 'rat_binnedMB_escape_scatter.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/escape/', device = 'pdf', 
#       useDingbats = F, height = 4, width = 6)

rat_binned_chrom_df = binned_chrom_df
save(rat_binned_chrom_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/rat_binned_chrom_df.Rdata')
```
...
bad_rat_coordinates = unlist(lapply(1:length(bad_rat_bins), function(i) ((bad_rat_bins - 1) *1e6)[i]:((bad_rat_bins)*1e6)[i]))
...
filter_bad_bins = function(data){
  if(is.null(dim(data))){return(NA)}
    ...
    escape_index = data$Pos %in% bad_rat_coordinates
    data = data[!escape_index, ]
    return(data)
  }
}

filt_rat_vcfs_noBadbins = mclapply(filt_rat_vcfs, filter_bad_bins, mc.cores = 10)
save(filt_rat_vcfs_noBadbins, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/max_snp_vcf_binFilt_4_27_23.Rdata')
```
...
num_snp_good_bins = vector(mode = 'numeric', length = length(filt_rat_vcfs_noBadbins))
for(i in 1:length(filt_rat_vcfs_noBadbins)){
  
  data = filt_rat_vcfs_noBadbins[[i]]
...
```{r}
Ns.escape = vector(mode='numeric', length = length(filt_rat_vcfs_noBadbins))

for(i in 1:length(filt_rat_vcfs_noBadbins)){
  
  suppressWarnings(if(is.null(dim(filt_rat_vcfs_noBadbins[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_rat_vcfs_noBadbins[[i]])[1]})
...
for(i in 1: length(filt_rat_vcfs_noBadbins)){
  ...
  total_exp = filt_rat_vcfs_noBadbins[[i]]$ref_counts + filt_rat_vcfs_noBadbins[[i]]$alt_counts
  ref_ratios = filt_rat_vcfs_noBadbins[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_rat_vcfs_noBadbins), function(i) 
  est_skew_func(filt_rat_vcfs_noBadbins[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
for(i in 1:length(sample_index)){
  ...
  data = filt_rat_vcfs_noBadbins[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
escape_mean = rep(NA, length = length(annot_rat_vcfs))
escape_var = rep(NA, length = length(annot_rat_vcfs))
num_escape_snps = rep(NA, length = length(annot_rat_vcfs))
inact_mean = rep(NA, length = length(annot_rat_vcfs))
inact_var = rep(NA, length = length(annot_rat_vcfs))
num_inact_snps = rep(NA, length = length(annot_rat_vcfs))

for(i in 1:length(annot_rat_vcfs)){

  data = annot_rat_vcfs[[i]]
  if(is.null(dim(data))){next}
  escape_index = (plyr::round_any(data$Pos, 1000000, ceiling)/1e6) %in% bad_rat_bins #Get the SNPs located in the escape bins
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
hist(inact_mean[inact_mean >= .10] - escape_mean[inact_mean >= .10], breaks = 40)

maybe_ref_biased = which(inact_mean >= .10  & (inact_mean - escape_mean) >= -.05  & (inact_mean - escape_mean) <= .05 )
...
chrX_skewed_index = which(est_skew_no_escape >= .6 & num_snp_good_bins >= 10)

all_chrX_skewed_snps = c()
for(i in 1:length(chrX_skewed_index)){
  
  data = filt_rat_vcfs_noBadbins[[chrX_skewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
chrX_Nonskewed_index = which(est_skew_no_escape < .6 & num_snp_good_bins >= 10)

all_chrX_Nonskewed_snps = c()
for(i in 1:length(chrX_Nonskewed_index)){
  
  data = filt_rat_vcfs_noBadbins[[chrX_Nonskewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(enoughSNPs_index)){
  ...
  data = filt_rat_vcfs_noBadbins[[enoughSNPs_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  all_ref_ratios = c(all_ref_ratios, ref_ratios)
...
ggplot(skew_bin_snp_df, aes(x = ref_ratios, y = bin_label)) + geom_density_ridges(rel_min_height = 0.01, scale = 0.95, fill = species_colors[2] ) + xlim(0,1) +
...
#ggsave(filename = 'rat_chrX_skew_binned_refRatio_ridgePlot.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/species_skew_binned_refRatios/', 
#       device = 'pdf', useDingbats = F, height = 6, width = 6)

rat_skew_bin_snp_df = skew_bin_snp_df
save(rat_skew_bin_snp_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/rat_skew_bin_snp_df.Rdata')
```



```{r}
rat_skew_and_stats_df = data.frame(sample_id = wig_samples, 
...
rat_skew_and_stats_df
```


```{r}

save(rat_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/rat_skew_and_stats_binFilt_df_4_27_23.Rdata')
...
all_ref_ratios = c()
all_snp_pos = c()
rat_het_index = which(est_skew_no_escape >= .6)
for(i in 1:length(rat_het_index)){

  data = filt_rat_vcfs_noBadbins[[rat_het_index[i]]]
  if(is.null(dim(data))){next}
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts /(data$ref_counts + data$alt_counts)
...
For rat, not powered to call escape like usual, filtering out too many SNPs from reference bias to leave enough to call escape.
Include the reference biased data to find escape and lower the thresholds for this species. Will have more false positives for escape
...
for(i in 1:length(filt_rat_vcfs)){
  if(is.null(dim(filt_rat_vcfs[[i]]))){next}
  all_genes = c(all_genes,filt_rat_vcfs[[i]]$gene )
}
...
skewed_index = which(est_skew_bad_snps >= .55)
...
for(j in 1:length(all_genes)){
  ...
  for(i in 1:length(skewed_index)){
    ...
    if(is.null(dim(filt_rat_vcfs[[skewed_index[i]]]))){next}
    
    data = filt_rat_vcfs[[skewed_index[i]]]
    #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
sample_index = which(total_snps >= 10 & est_skew_bad_snps > .55)
all_escape_ratios = c()
all_non_ratios  = c()
for(i in 1:length(sample_index)){
  
  data = filt_rat_vcfs[[sample_index[i]]]
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
escape_rat_df = data.frame(gene = all_genes, num_pvalues = num_pvalues,
...
save(escape_rat_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/escape_rat_df_3_15_23.Rdata')
...
escape_genes = escape_rat_df$gene[escape_rat_df$escape_label == 'escape']
...
for(i in 1:nrow(snp_stat_df)){
  gene_index = findInterval(snp_stat_df$snp_pos[i], rat_x_meta$start)
  if(gene_index == 0){gene_name_vec[i] = NA; next}
  
  if(snp_stat_df$snp_pos[i] <= rat_x_meta$end[gene_index]){
    gene_name_vec[i] = rat_x_meta$gene[gene_index]
...
gene_pos_df = data.frame(gene_name = rat_x_meta$gene, gene_start = rat_x_meta$start, gene_end = rat_x_meta$end, 
                         gene_biotype = rat_x_meta$gene_biotype, escape_label = rat_x_meta$gene %in% escape_genes)
...
```{r}
filt_rat_vcfs_noEscape = filt_rat_vcfs
num_snp_escape = vector(mode = 'numeric', length = length(filt_rat_vcfs_noEscape))
for(i in 1:length(filt_rat_vcfs_noEscape)){
  
  if(is.null(dim(filt_rat_vcfs_noEscape[[i]]))){next}
  escape_index = filt_rat_vcfs_noEscape[[i]]$gene %in% escape_genes
  filt_rat_vcfs_noEscape[[i]] = filt_rat_vcfs_noEscape[[i]][!escape_index, ]
  num_snp_escape[i] = dim(filt_rat_vcfs_noEscape[[i]])
}
...
save(filt_rat_vcfs_noEscape, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/max_snp_vcf_noEscape_3_15_23.Rdata')

```

```{r}
load(file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/max_snp_vcf_noEscape.Rdata')
filt_rat_vcfs_noEscape[1:5]
```


```{r}
Ns.escape = vector(mode='numeric', length = length(filt_rat_vcfs_noEscape))

for(i in 1:length(filt_rat_vcfs_noEscape)){
  
  suppressWarnings(if(is.null(dim(filt_rat_vcfs_noEscape[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_rat_vcfs_noEscape[[i]])[1]})
...
for(i in 1: length(filt_rat_vcfs_noEscape)){
  ...
  total_exp = filt_rat_vcfs_noEscape[[i]]$ref_counts + filt_rat_vcfs_noEscape[[i]]$alt_counts
  ref_ratios = filt_rat_vcfs_noEscape[[i]]$ref_counts / total_exp
  ratios.escape.max[[i]] = ref_ratios
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_rat_vcfs_noEscape), function(i) 
  est_skew_func(filt_rat_vcfs_noEscape[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
num_good_snps_escape = vector(length = length(filt_rat_vcfs_noEscape), mode = 'numeric')

for(i in 1:length(filt_rat_vcfs_noEscape)){
  
  data = filt_rat_vcfs_noEscape[[i]]
...
length(escape_genes)
sample_index = which(est_skew_no_escape >= .65 & num_good_snps_escape >= 10)

for(i in 1:length(sample_index)){
  
  data = filt_rat_vcfs_noEscape[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
##################################
looking at skew distributions

#################################

```{r}
quantile(est_var_no_escape, probs = seq(0,1,.05), na.rm = T)
sample_index = num_good_snps_escape >= 10 & est_var_no_escape < .14
sum(sample_index)
hist(unfold(est_skew_no_escape[num_good_snps_escape >= 10 & est_var_no_escape < .14]), breaks = seq(0, 1, .01))


```
```{r}

sample_index = which(num_good_snps_escape >= 10 & est_var_no_escape < .14)

for(i in 1:length(sample_index)){
  data = filt_rat_vcfs_noEscape[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  hist(ratios, breaks = seq(0,1,.025), main = est_skew[sample_index[i]])
...
nohup Rscript bootstrapping_script.R /path_to_species_filt_max_vcf /path_to_snp_df /path_to_save_boot_results &


```{r}

load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/bootstrapping_results_3_15_23.Rdata')
length(boots)
boots[1:5]
```
...
plot(est_var_no_escape, CI_width)
```

```{r}

sample_index = which(num_good_snps_escape >= 10 & CI_width >= .213 & est_var_no_escape >= .14 )
length(sample_index)
for(i in 1:length(sample_index)){
  data = filt_rat_vcfs_noEscape[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
sample_index = which(num_good_snps >= 10)
hist(unfold(est_skew[sample_index]), breaks = seq(0,1,.01))
```
...
save(rat_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/rat_skew_and_stats_df_3_15_23.Rdata')

```

```{r}

load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/rattus_norvegicus/rat_skew_and_stats_df_3_15_23.Rdata')
...
sum(rat_skew_and_stats_df$num_good_snps >= 10)
hist(rat_skew_and_stats_df$est_skew[rat_skew_and_stats_df$num_good_snps >= 10], breaks = seq(.5,1, .005))
...
```{r}
num_samples = length(filt_rat_vcfs)
...
ggplot(snp_stat_df[snp_filter, ], aes(x = snp_pos, y = prop_samples_present)) + geom_point(alpha = .5, size = .5) + xlab('X chromosome coordinate') + 
  ylab('% of samples SNP is present and passed QC') + ggtitle(sprintf('Het SNPs (%i) present in Rattus norvegicus bulk RNA-seq samples (%i) ',num_snps_present, num_samples))


ggplot(snp_stat_df[snp_filter, ], aes(x = snp_pos, y = mean_total_counts)) + geom_point(alpha = .5, size = .5) + xlab('X chromosome coordinate') + 
  ylab('mean total reads') + ggtitle(sprintf('Het SNPs (%i) present in Rattus norvegicus bulk RNA-seq samples (%i) ',num_snps_present, num_samples))
...
p1 = ggplot(rat_skew_and_stats_df, aes(x = est_skew, y = num_good_snps)) + geom_point(alpha = .5, size = .5) + xlab('Estimated XCI skew') + ylab("Number of SNPs skew estimate is derived from") + ggtitle('Rattus norvegicus samples') + xlim(.5,1)
