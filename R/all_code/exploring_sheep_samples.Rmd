...
'/grid/gillis/data/hover/work/cmxci.paired/Ovis_aries'
'/grid/gillis/data/hover/work/cmxci.single/Ovis_aries'
...
star_output = paste("/grid/gillis/home/hover/work/cmxci.paired/Ovis_aries/", star_output, sep = '')
star_output
```
...
high_snp_index = which(num_snps >= 1000)
...
annot_sheep_vcfs = wasp_filt_vcf

for(j in 1:length(annot_sheep_vcfs)){
  test = annot_sheep_vcfs[[j]]
  ...
  for( i in 1:dim(test)[1]){
    ...
    if(test_gene_start <= x_gene_ends[gene_index]){
      ...
      gene_annots[i] = sheep_x_meta$gene[gene_index]
      gene_biotype[i] = sheep_x_meta$gene_biotype[gene_index]
    }
  }
  
  test$gene = gene_annots
  test$gene_biotype = gene_biotype
  annot_sheep_vcfs[[j]] = test
}

head(annot_sheep_vcfs)
...
```{r}
#save(annot_sheep_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/all_snp_vcf.Rdata')
save(annot_sheep_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/all_snp_vcf_3_15_23.Rdata')
...
captured_genes = c()
for( i in 1:length(annot_sheep_vcfs)){
  suppressWarnings(if(is.null(dim(annot_sheep_vcfs[[i]]))){next})
  captured_genes = c(captured_genes, annot_sheep_vcfs[[i]]$gene)
}
...
for(i in 201:300){
  ...
  gene_ref_exp = c()
  for(j in 1:length(annot_sheep_vcfs)){
    suppressWarnings(if(is.null(dim(annot_sheep_vcfs[[j]]))){next})
    gene_index = annot_sheep_vcfs[[j]]$gene == current_gene
    ref_counts = annot_sheep_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_sheep_vcfs[[j]]$alt_counts[gene_index]
...
for( i in 1:length(annot_sheep_vcfs)){
  suppressWarnings(if(is.null(dim(annot_sheep_vcfs[[i]]))){next})
  captured_SNPs = c(captured_SNPs, annot_sheep_vcfs[[i]]$Pos)
}
...
for(i in 1:length(captured_SNPs)){
  ...
  for(j in 1:length(annot_sheep_vcfs)){
    suppressWarnings(if(is.null(dim(annot_sheep_vcfs[[j]]))){next})
    gene_index = annot_sheep_vcfs[[j]]$Pos == current_SNP
    ref_counts = annot_sheep_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_sheep_vcfs[[j]]$alt_counts[gene_index]
    total_counts = ref_counts + alt_counts
...
```{r}
hist(snp_stat_df$mean_ref_ratio, breaks = seq(0,1,.01), xlab = 'Mean reference ratio per SNP', main = 'Ovis aries')
...
plot(snp_stat_df$mean_ref_ratio,snp_stat_df$num_samples_present, cex = .25, xlab = 'Mean reference ratio', 
     ylab = '# of samples SNP is detected', main = 'Ovis aries Het SNPs')
abline(v = .5, col = 'red')
...
save(snp_stat_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/detected_het_snps_df_3_15_23.Rdata')

sheep_snp_stat_df = snp_stat_df
save(sheep_snp_stat_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/sheep_snp_stat_df.Rdata')
```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/detected_het_snps_df_3_15_23.Rdata')
...
#ggsave(plot = p3, filename = 'sheep_x_ref_snp_filtering_plots.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/ref_snp_filtering/', device = 'pdf',
...
```{r}
filt_sheep_vcfs = annot_sheep_vcfs

for(i in 1:length(filt_sheep_vcfs)){
  test = filt_sheep_vcfs[[i]]
  
  suppressWarnings(if(is.null(dim(test))){next})
  
  #Get rid of SNPs in nonannotated genes
  test = test[test$gene != "", ]
  suppressWarnings(if(dim(test)[1] == 0){filt_sheep_vcfs[[i]] = NA; next})
...
```{r}
save(filt_sheep_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/max_snp_vcf_3_15_23.Rdata')


```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/max_snp_vcf_3_15_23.Rdata')
...
```{r}
num_filt_genes = vector(mode = 'numeric', length = length(filt_sheep_vcfs))
for(i in 1:length(filt_sheep_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_sheep_vcfs[[i]]))){
    num_filt_genes[i] = 0
    next
  })
  num_filt_genes[i] = dim(filt_sheep_vcfs[[i]])[1]
}
```

```{r}
max(num_filt_genes)
hist(num_filt_genes, breaks = seq(0,310,10))
...
for(i in index){
  ...
  data = filt_sheep_vcfs[[index_num_samps[i]]]
...
```{r}
Ns = vector(mode='numeric', length = length(filt_sheep_vcfs))

for(i in 1:length(filt_sheep_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_sheep_vcfs[[i]]))){Ns[i] = 0}
  else{Ns[i] = dim(filt_sheep_vcfs[[i]])[1]})
...
for(i in 1: length(filt_sheep_vcfs)){
  ...
  total_exp = filt_sheep_vcfs[[i]]$ref_counts + filt_sheep_vcfs[[i]]$alt_counts
  ref_ratios = filt_sheep_vcfs[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits = mclapply(1:length(filt_sheep_vcfs), function(i) est_skew_func(filt_sheep_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()


Sys.time()
folded_norm_fits_bad_snps = mclapply(1:length(filt_sheep_vcfs), function(i) est_skew_func(filt_sheep_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df, filter_snps = F), mc.cores = 20)
Sys.time()
...
hist(est_skew, main = 'sheep skew estimates filtering out bad SNPs', breaks=64)
...
num_good_snps = vector(length = length(filt_sheep_vcfs), mode = 'numeric')
total_snps = vector(length = length(filt_sheep_vcfs), mode = 'numeric')

for(i in 1:length(filt_sheep_vcfs)){
  
  data = filt_sheep_vcfs[[i]]
...
all_ref_ratios = c()
all_snp_pos = c()
sheep_het_index = which(est_skew >= .7)
for(i in 1:length(sheep_het_index)){

  data = annot_sheep_vcfs[[sheep_het_index[i]]]
  if(is.null(dim(data))){next}
...
#Excluding the first 9MB as likely pseudo-autosomal regions
#And excluding 1MB bins with a mean reference ratio below .65
bad_sheep_bins = which(mean_bin_ref_ratio<= .65)
bad_sheep_bins = c(c(1,2,3,4,5,6,7,8,9), bad_sheep_bins)
bad_sheep_bins = unique(bad_sheep_bins)


binned_chrom_df$keep_label = rep('Kept 1MB bin', length = nrow(binned_chrom_df))
binned_chrom_df$keep_label[(binned_chrom_df$bin/1e6) %in% (bad_sheep_bins - 1)] = 'Excluded 1MB bin'
...
ggMarginal(p1, margins = 'y', type = 'histogram', yparams = list(binwidth = .005))
#ggsave(filename = 'sheep_binnedMB_escape_scatter.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/escape/', device = 'pdf', 
#       useDingbats = F, height = 4, width = 6)

sheep_binned_chrom_df = binned_chrom_df
save(sheep_binned_chrom_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/sheep_binned_chrom_df.Rdata')
```
...
bad_sheep_coordinates = unlist(lapply(1:length(bad_sheep_bins), function(i) ((bad_sheep_bins - 1)*1e6)[i]:((bad_sheep_bins)*1e6)[i]))
...
filter_bad_bins = function(data){
  if(is.null(dim(data))){return(NA)}
    ...
    escape_index = data$Pos %in% bad_sheep_coordinates
    data = data[!escape_index, ]
    return(data)
  }
}

filt_sheep_vcfs_noBadbins = mclapply(filt_sheep_vcfs, filter_bad_bins, mc.cores = 10)
save(filt_sheep_vcfs_noBadbins, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/max_snp_vcf_binFilt_4_27_23.Rdata')
```
...
num_snp_good_bins = vector(mode = 'numeric', length = length(filt_sheep_vcfs_noBadbins))
for(i in 1:length(filt_sheep_vcfs_noBadbins)){
  
  data = filt_sheep_vcfs_noBadbins[[i]]
...
```{r}
Ns.escape = vector(mode='numeric', length = length(filt_sheep_vcfs_noBadbins))

for(i in 1:length(filt_sheep_vcfs_noBadbins)){
  
  suppressWarnings(if(is.null(dim(filt_sheep_vcfs_noBadbins[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_sheep_vcfs_noBadbins[[i]])[1]})
...
for(i in 1: length(filt_sheep_vcfs_noBadbins)){
  ...
  total_exp = filt_sheep_vcfs_noBadbins[[i]]$ref_counts + filt_sheep_vcfs_noBadbins[[i]]$alt_counts
  ref_ratios = filt_sheep_vcfs_noBadbins[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_sheep_vcfs_noBadbins), function(i) 
  est_skew_func(filt_sheep_vcfs_noBadbins[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
for(i in 1:length(sample_index)){
  ...
  data = filt_sheep_vcfs_noBadbins[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(chrX_skewed_index)){
  ...
  data = filt_sheep_vcfs_noBadbins[[chrX_skewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(chrX_Nonskewed_index)){
  ...
  data = filt_sheep_vcfs_noBadbins[[chrX_Nonskewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(enoughSNPs_index)){
  ...
  data = filt_sheep_vcfs_noBadbins[[enoughSNPs_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  all_ref_ratios = c(all_ref_ratios, ref_ratios)
...
ggplot(skew_bin_snp_df, aes(x = ref_ratios, y = bin_label)) + geom_density_ridges(rel_min_height = 0.01, scale = 0.95, fill = species_colors[6] ) + xlim(0,1) +
...
#ggsave(filename = 'sheep_chrX_skew_binned_refRatio_ridgePlot.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/species_skew_binned_refRatios/', 
#       device = 'pdf', useDingbats = F, height = 6, width = 6)

sheep_skew_bin_snp_df = skew_bin_snp_df
save(sheep_skew_bin_snp_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/sheep_skew_bin_snp_df.Rdata')
```



```{r}
sheep_skew_and_stats_df = data.frame(sample_id = wig_samples, 
...
sheep_skew_and_stats_df
```


```{r}

save(sheep_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/sheep_skew_and_stats_binFilt_df_4_27_23.Rdata')
...
all_ref_ratios = c()
all_snp_pos = c()
sheep_het_index = which(est_skew_no_escape >= .7)
for(i in 1:length(sheep_het_index)){

  data = filt_sheep_vcfs_noBadbins[[sheep_het_index[i]]]
  if(is.null(dim(data))){next}
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts /(data$ref_counts + data$alt_counts)
...
for(i in 1:length(filt_sheep_vcfs)){
  if(is.null(dim(filt_sheep_vcfs[[i]]))){next}
  all_genes = c(all_genes,filt_sheep_vcfs[[i]]$gene )
}
...
for(j in 1:length(all_genes)){
  ...
  for(i in 1:length(skewed_index)){
    ...
    if(is.null(dim(filt_sheep_vcfs[[skewed_index[i]]]))){next}
    
    data = filt_sheep_vcfs[[skewed_index[i]]]
    #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
for(i in 1:length(sample_index)){
  ...
  data = filt_sheep_vcfs[[sample_index[i]]]
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
escape_sheep_df = data.frame(gene = all_genes, num_pvalues = num_pvalues,
...
save(escape_sheep_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/escape_sheep_df_3_15_23.Rdata')
...
load( '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/escape_sheep_df.Rdata')

sheep_escape = escape_sheep_df$gene[escape_sheep_df$escape_label == 'escape']
...
```{r}
filt_sheep_vcfs_noEscape = filt_sheep_vcfs
num_snp_escape = vector(mode = 'numeric', length = length(filt_sheep_vcfs_noEscape))
for(i in 1:length(filt_sheep_vcfs_noEscape)){
  
  if(is.null(dim(filt_sheep_vcfs_noEscape[[i]]))){next}
  escape_index = filt_sheep_vcfs_noEscape[[i]]$gene %in% escape_genes
  filt_sheep_vcfs_noEscape[[i]] = filt_sheep_vcfs_noEscape[[i]][!escape_index, ]
  num_snp_escape[i] = dim(filt_sheep_vcfs_noEscape[[i]])
}


save(filt_sheep_vcfs_noEscape, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/max_snp_vcf_noEscape_3_15_23.Rdata')
...
length(filt_sheep_vcfs_noEscape)
hist(num_snp_escape)
```
...
```{r}
load(file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/max_snp_vcf_noEscape.Rdata')
filt_sheep_vcfs_noEscape[1:5]
```


```{r}
Ns.escape = vector(mode='numeric', length = length(filt_sheep_vcfs_noEscape))

for(i in 1:length(filt_sheep_vcfs_noEscape)){
  
  suppressWarnings(if(is.null(dim(filt_sheep_vcfs_noEscape[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_sheep_vcfs_noEscape[[i]])[1]})
...
for(i in 1: length(filt_sheep_vcfs_noEscape)){
  ...
  total_exp = filt_sheep_vcfs_noEscape[[i]]$ref_counts + filt_sheep_vcfs_noEscape[[i]]$alt_counts
  ref_ratios = filt_sheep_vcfs_noEscape[[i]]$ref_counts / total_exp
  ratios.escape.max[[i]] = ref_ratios
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_sheep_vcfs_noEscape), function(i) 
  est_skew_func(filt_sheep_vcfs_noEscape[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
num_good_snps_escape = vector(length = length(filt_sheep_vcfs_noEscape), mode = 'numeric')

for(i in 1:length(filt_sheep_vcfs_noEscape)){
  
  data = filt_sheep_vcfs_noEscape[[i]]
...
escape_genes = escape_sheep_df$gene[escape_sheep_df$escape_label == 'escape']
...
for(i in 1:nrow(snp_stat_df)){
  gene_index = findInterval(snp_stat_df$snp_pos[i], sheep_x_meta$start)
  if(gene_index == 0){gene_name_vec[i] = NA; next}
  
  if(snp_stat_df$snp_pos[i] <= sheep_x_meta$end[gene_index]){
    gene_name_vec[i] = sheep_x_meta$gene[gene_index]
...
gene_pos_df = data.frame(gene_name = sheep_x_meta$gene, gene_start = sheep_x_meta$start, gene_end = sheep_x_meta$end, 
                         gene_biotype = sheep_x_meta$gene_biotype, escape_label = sheep_x_meta$gene %in% escape_genes)
...
potential pseudo autosomal genes, any SNP that comes before
...
ggplot(gene_pos_df, aes(x = gene_start, y = log10(mean_exp), color = escape_label)) + geom_point(aes(size = escape_label)) +
  geom_vline(xintercept = 12000000, col = 'red') +
  scale_color_manual(values = c('TRUE' = 'red', 'FALSE' = 'black')) +
  scale_size_manual(values = c('TRUE' = 2, 'FALSE' = 1))

```
check out the potential psuedo autosomal genes expression ratios
```{r}
pseudo_end = 11798562
sample_index = which(sheep_skew_and_stats_no_escape_df$est_skew_no_escape > .6 & sheep_skew_and_stats_no_escape_df$num_good_snps_no_escape >= 10)
length(sample_index)


for(i in 1:length(sample_index)){
  
  data = filt_sheep_vcfs_noEscape[[sample_index[i]]]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  pseudo_label = data$Pos <= pseudo_end
  hist(ref_ratios[!pseudo_label], breaks = seq(0,1, .025))
  hist(ref_ratios[pseudo_label], breaks = seq(0,1, .025), col = 'red', add = T)
...
sheep genes in that potential pseudo autosomal region do not look like strong escapers at all in skewed samples


```{r}

quantile(est_var_no_escape, probs = seq(0,1,.10), na.rm = T)
sample_index = which(num_good_snps_escape >= 10 & est_var_no_escape < .12)
...
Redo the bootstrap without escape genes


```{r}

load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/bootstrapping_results_3_15_23.Rdata')
length(boots)
boots[1:5]
```
...
```{r}
sheep_skew_and_stats_df = data.frame(sample_id = wig_samples, est_skew = est_skew, est_var = est_var,
...
save(sheep_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/sheep_skew_and_stats_df_3_15_23.Rdata')

```

```{r}

load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/ovis_aries/sheep_skew_and_stats_df_3_15_23.Rdata')
...
sum(sheep_skew_and_stats_df$num_good_snps_no_escape >= 10)
hist(sheep_skew_and_stats_df$est_skew_no_escape[sheep_skew_and_stats_df$num_good_snps_no_escape >= 10], breaks = seq(.5,1, .005))
...
```{r}
num_samples = length(filt_sheep_vcfs)
...
ggplot(snp_stat_df[snp_filter, ], aes(x = snp_pos, y = prop_samples_present)) + geom_point(alpha = .5, size = .5) + xlab('X chromosome coordinate') + 
  ylab('% of samples SNP is present and passed QC') + ggtitle(sprintf('Het SNPs (%i) present in Ovis aries bulk RNA-seq samples (%i) ',num_snps_present, num_samples))


ggplot(snp_stat_df[snp_filter, ], aes(x = snp_pos, y = mean_total_counts)) + geom_point(alpha = .5, size = .5) + xlab('X chromosome coordinate') + 
  ylab('mean total reads') + ggtitle(sprintf('Het SNPs (%i) present in Ovis aries bulk RNA-seq samples (%i) ',num_snps_present, num_samples))
...
p1 = ggplot(sheep_skew_and_stats_df, aes(x = est_skew, y = num_good_snps)) + geom_point(alpha = .5, size = .5) + xlab('Estimated XCI skew') + ylab("Number of SNPs skew estimate is derived from") + ggtitle('Ovis aries samples') + xlim(.5,1)
...
plot(sheep_skew_and_stats_df$CI_width, sheep_skew_and_stats_df$est_var_no_escape)
...
sample_index = which(num_good_snps_escape >= 10 & CI_width > .152)

for(i in 1:length(sample_index)){
  data = filt_sheep_vcfs_noEscape[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  hist(ref_ratios, breaks = seq(0,1,.025), main = sprintf('skew: %1.3f CI: %1.3f', est_skew_no_escape[sample_index[i]], CI_width[sample_index[i]]))
...
sample_index = sheep_skew_and_stats_no_escape_df$num_good_snps_no_escape >= 10
hist(sheep_skew_and_stats_no_escape_df$CI_width_no_escape[sample_index], breaks = 32)

```

```{r}
quantile(sheep_skew_and_stats_df$est_var_no_escape, probs = seq(0,1,.10), na.rm = T)
sample_index = sheep_skew_and_stats_df$num_good_snps_no_escape >= 10 & sheep_skew_and_stats_df$est_var_no_escape < .13
sum(sample_index)
hist(sheep_skew_and_stats_df$est_skew_no_escape[sample_index], breaks = seq(.5, 1, .005))

hist(unfold(sheep_skew_and_stats_df$est_skew_no_escape[sample_index]), breaks = seq(0, 1, .01), 
     main = 'sheep', xlab = 'Unfolded population skews')

```

###################################
Cell number modeling
