...
Checking out the mouse samples
...
wasp_vcf_files_paired = processed_files[grepl('.wasp.X.snps_filtered.vcf',processed_files  )]
#Ignore the .vcf.idx files
wasp_vcf_files_paired = wasp_vcf_files_paired[!grepl('.idx',wasp_vcf_files_paired )]
wasp_wig_files_paired = processed_files[grepl('.wasp.X.split.filtered.wig',processed_files  )]
...
wasp_vcf_files_single = processed_files[grepl('.wasp.X.snps_filtered.vcf',processed_files  )]
#Ignore the .vcf.idx files
wasp_vcf_files_single = wasp_vcf_files_single[!grepl('.idx',wasp_vcf_files_single )]
wasp_wig_files_single = processed_files[grepl('.wasp.X.split.filtered.wig',processed_files  )]
...
high_snp_index = which(num_snps >= 2000)
...
annot_mouse_vcfs = wasp_filt_vcf

for(j in 1:length(annot_mouse_vcfs)){
  test = annot_mouse_vcfs[[j]]
  ...
  for( i in 1:dim(test)[1]){
    ...
    if(test_gene_start <= x_gene_ends[gene_index]){
      ...
      gene_annots[i] = mouse_x_meta$gene[gene_index]
      gene_biotype[i] = mouse_x_meta$gene_biotype[gene_index]
    }
  }
  
  test$gene = gene_annots
  test$gene_biotype = gene_biotype
  annot_mouse_vcfs[[j]] = test
}

head(annot_mouse_vcfs)
...
```{r}
save(annot_mouse_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/all_snp_vcf_4_22_24.Rdata')
...
captured_genes = c()
for( i in 1:length(annot_mouse_vcfs)){
  suppressWarnings(if(is.null(dim(annot_mouse_vcfs[[i]]))){next})
  captured_genes = c(captured_genes, annot_mouse_vcfs[[i]]$gene)
}
...
for(i in 1:100){
  ...
  gene_ref_exp = c()
  for(j in 1:length(annot_mouse_vcfs)){
    suppressWarnings(if(is.null(dim(annot_mouse_vcfs[[j]]))){next})
    gene_index = annot_mouse_vcfs[[j]]$gene == current_gene
    ref_counts = annot_mouse_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_mouse_vcfs[[j]]$alt_counts[gene_index]
...
for( i in 1:length(annot_mouse_vcfs)){
  suppressWarnings(if(is.null(dim(annot_mouse_vcfs[[i]]))){next})
  captured_SNPs = c(captured_SNPs, annot_mouse_vcfs[[i]]$Pos)
}
...
for(i in 1:length(captured_SNPs)){
  ...
  for(j in 1:length(annot_mouse_vcfs)){
    suppressWarnings(if(is.null(dim(annot_mouse_vcfs[[j]]))){next})
    gene_index = annot_mouse_vcfs[[j]]$Pos == current_SNP
    ref_counts = annot_mouse_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_mouse_vcfs[[j]]$alt_counts[gene_index]
    total_counts = ref_counts + alt_counts
...
```{r}
hist(snp_stat_df$mean_ref_ratio, breaks = seq(0,1,.01), xlab = 'Mean reference ratio per SNP', main = 'Mouse')
...
plot(snp_stat_df$mean_ref_ratio,snp_stat_df$num_samples_present, cex = .25, xlab = 'Mean reference ratio', 
     ylab = '# of samples SNP is detected', main = 'Mouse Het SNPs')
abline(v = .5, col = 'red')
...
save(snp_stat_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/detected_het_snps_df_4_22_24.Rdata')

mouse_snp_stat_df = snp_stat_df
save(mouse_snp_stat_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/mouse_snp_stat_df.Rdata')
```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/detected_het_snps_df_4_22_24.Rdata')
...
#ggsave(plot = p3, filename = 'mouse_x_ref_snp_filtering_plots.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/ref_snp_filtering/', device = 'pdf',
...
```{r}
filt_mouse_vcfs = annot_mouse_vcfs

for(i in 1:length(filt_mouse_vcfs)){
  test = filt_mouse_vcfs[[i]]
  
  suppressWarnings(if(is.null(dim(test))){next})
  
  #Get rid of SNPs in nonannotated genes
  test = test[test$gene != "", ]
  suppressWarnings(if(dim(test)[1] == 0){filt_mouse_vcfs[[i]] = NA; next})
...
```{r}
save(filt_mouse_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/max_snp_vcf_4_22_24.Rdata')


```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/max_snp_vcf_4_22_24.Rdata')
...
num_filt_genes = vector(mode = 'numeric', length = length(filt_mouse_vcfs))
for(i in 1:length(filt_mouse_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_mouse_vcfs[[i]]))){
    num_filt_genes[i] = 0
    next
    
  })
  
  num_filt_genes[i] = dim(filt_mouse_vcfs[[i]])[1]
...
for(i in index){
  ...
  data = filt_mouse_vcfs[[index_num_samps[i]]]
...
```{r}
Ns = vector(mode='numeric', length = length(filt_mouse_vcfs))

for(i in 1:length(filt_mouse_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_mouse_vcfs[[i]]))){Ns[i] = 0}
  else{Ns[i] = dim(filt_mouse_vcfs[[i]])[1]})
...
for(i in 1: length(filt_mouse_vcfs)){
  ...
  total_exp = filt_mouse_vcfs[[i]]$ref_counts + filt_mouse_vcfs[[i]]$alt_counts
  ref_ratios = filt_mouse_vcfs[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits = mclapply(1:length(filt_mouse_vcfs), function(i) est_skew_func(filt_mouse_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()


Sys.time()
folded_norm_fits_bad_snps = mclapply(1:length(filt_mouse_vcfs), function(i) est_skew_func(filt_mouse_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df, filter_snps = F), mc.cores = 20)
Sys.time()
...
hist(est_skew, main = 'mouse skew estimates filtering out bad SNPs', breaks=64)
...
num_good_snps = vector(length = length(filt_mouse_vcfs), mode = 'numeric')
total_snps = vector(length = length(filt_mouse_vcfs), mode = 'numeric')

for(i in 1:length(filt_mouse_vcfs)){
  
  data = filt_mouse_vcfs[[i]]
...
all_ref_ratios = c()
all_snp_pos = c()
mouse_het_index = which(est_skew >= .7)
for(i in 1:length(mouse_het_index)){

  data = annot_mouse_vcfs[[mouse_het_index[i]]]
  if(is.null(dim(data))){next}
...
#Excluding the last 3MB as likely pseudo-autosomal regions
#And excluding 1MB bins with a mean reference ratio below .65
bad_mouse_bins = which(mean_bin_ref_ratio<= .65)
bad_mouse_bins = c( bad_mouse_bins, c(167, 168, 169))
bad_mouse_bins = unique(bad_mouse_bins)


binned_chrom_df$keep_label = rep('Kept 1MB bin', length = nrow(binned_chrom_df))
binned_chrom_df$keep_label[(binned_chrom_df$bin/1e6) %in% (bad_mouse_bins - 1)] = 'Excluded 1MB bin'
...
ggMarginal(p1, margins = 'y', type = 'histogram', yparams = list(binwidth = .005))
#ggsave(filename = 'mouse_binnedMB_escape_scatter.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/escape/', device = 'pdf', 
#       useDingbats = F, height = 4, width = 6)

mouse_binned_chrom_df = binned_chrom_df
save(mouse_binned_chrom_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/mouse_binned_chrom_df.Rdata')
```
...
bad_mouse_coordinates = unlist(lapply(1:length(bad_mouse_bins), function(i) ((bad_mouse_bins - 1)*1e6)[i]:((bad_mouse_bins)*1e6)[i]))
...
filter_bad_bins = function(data){
  if(is.null(dim(data))){return(NA)}
    ...
    escape_index = data$Pos %in% bad_mouse_coordinates
    data = data[!escape_index, ]
    return(data)
  }
}

filt_mouse_vcfs_noBadbins = mclapply(filt_mouse_vcfs, filter_bad_bins, mc.cores = 10)
save(filt_mouse_vcfs_noBadbins, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/max_snp_vcf_binFilt_4_22_24.Rdata')
```
...
num_snp_good_bins = vector(mode = 'numeric', length = length(filt_mouse_vcfs_noBadbins))
for(i in 1:length(filt_mouse_vcfs_noBadbins)){
  
  data = filt_mouse_vcfs_noBadbins[[i]]
...
```{r}
Ns.escape = vector(mode='numeric', length = length(filt_mouse_vcfs_noBadbins))

for(i in 1:length(filt_mouse_vcfs_noBadbins)){
  
  suppressWarnings(if(is.null(dim(filt_mouse_vcfs_noBadbins[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_mouse_vcfs_noBadbins[[i]])[1]})
...
for(i in 1: length(filt_mouse_vcfs_noBadbins)){
  ...
  total_exp = filt_mouse_vcfs_noBadbins[[i]]$ref_counts + filt_mouse_vcfs_noBadbins[[i]]$alt_counts
  ref_ratios = filt_mouse_vcfs_noBadbins[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_mouse_vcfs_noBadbins), function(i) 
  est_skew_func(filt_mouse_vcfs_noBadbins[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
for(i in 1:100){
  ...
  data = filt_mouse_vcfs_noBadbins[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
use the full annot_mouse_vcf to look across all snps in the sample
...
escape_mean = rep(NA, length = length(annot_mouse_vcfs))
escape_var = rep(NA, length = length(annot_mouse_vcfs))
num_escape_snps = rep(NA, length = length(annot_mouse_vcfs))
inact_mean = rep(NA, length = length(annot_mouse_vcfs))
inact_var = rep(NA, length = length(annot_mouse_vcfs))
num_inact_snps = rep(NA, length = length(annot_mouse_vcfs))

for(i in 1:length(annot_mouse_vcfs)){

  data = annot_mouse_vcfs[[i]]
  if(is.null(dim(data))){next}
  escape_index = (plyr::round_any(data$Pos, 1000000, ceiling)/1e6) %in% bad_mouse_bins #Get the SNPs located in the escape bins
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(chrX_skewed_index)){
  ...
  data = filt_mouse_vcfs_noBadbins[[chrX_skewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(chrX_Nonskewed_index)){
  ...
  data = filt_mouse_vcfs_noBadbins[[chrX_Nonskewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(enoughSNPs_index)){
  ...
  data = filt_mouse_vcfs_noBadbins[[enoughSNPs_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  all_ref_ratios = c(all_ref_ratios, ref_ratios)
...
ggplot(skew_bin_snp_df, aes(x = ref_ratios, y = bin_label)) + geom_density_ridges(rel_min_height = 0.01, scale = 0.95, fill = species_colors[7] ) + xlim(0,1) +
...
#ggsave(filename = 'mouse_chrX_skew_binned_refRatio_ridgePlot.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/species_skew_binned_refRatios/', 
#       device = 'pdf', useDingbats = F, height = 6, width = 6)

mouse_skew_bin_snp_df = skew_bin_snp_df
save(mouse_skew_bin_snp_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/mouse_skew_bin_snp_df.Rdata')
```



```{r}
mouse_skew_and_stats_df = data.frame(sample_id = wig_samples, 
...
mouse_skew_and_stats_df
```


```{r}

save(mouse_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/mouse_skew_and_stats_binFilt_df_4_22_24.Rdata')
...
all_ref_ratios = c()
all_snp_pos = c()
mouse_het_index = which(est_skew_no_escape >= .7)
for(i in 1:length(mouse_het_index)){

  data = filt_mouse_vcfs_noBadbins[[mouse_het_index[i]]]
  if(is.null(dim(data))){next}
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts /(data$ref_counts + data$alt_counts)
