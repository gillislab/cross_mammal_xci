...
for(i in 1:length(filt_pig_vcfs)){
  if(is.null(dim(filt_pig_vcfs[[i]]))){next}
  all_genes = c(all_genes,filt_pig_vcfs[[i]]$gene )
}
...
for(j in 1:length(all_genes)){
  ...
  for(i in 1:length(skewed_index)){
    ...
    if(is.null(dim(filt_pig_vcfs[[skewed_index[i]]]))){next}
    
    data = filt_pig_vcfs[[skewed_index[i]]]
    #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
for(i in 1:length(sample_index)){
  ...
  data = filt_pig_vcfs[[sample_index[i]]]
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
escape_pig_df = data.frame(gene = all_genes, num_pvalues = num_pvalues,
...
save(escape_pig_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/sus_scrofa/escape_pig_df_3_15_23.Rdata')
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/sus_scrofa/escape_pig_df.Rdata')
pig_escape = escape_pig_df$gene[escape_pig_df$escape_label == 'escape']

conserved_escape = intersect(pig_escape, sheep_escape)
...
escape_genes = escape_pig_df$gene[escape_pig_df$escape_label == 'escape']
...
for(i in 1:nrow(snp_stat_df)){
  gene_index = findInterval(snp_stat_df$snp_pos[i], pig_x_meta$start)
  if(gene_index == 0){gene_name_vec[i] = NA; next}
  
  if(snp_stat_df$snp_pos[i] <= pig_x_meta$end[gene_index]){
    gene_name_vec[i] = pig_x_meta$gene[gene_index]
...
gene_pos_df = data.frame(gene_name = pig_x_meta$gene, gene_start = pig_x_meta$start, gene_end = pig_x_meta$end, 
                         gene_biotype = pig_x_meta$gene_biotype, escape_label = pig_x_meta$gene %in% escape_genes)
...
```{r}
filt_pig_vcfs_noEscape = filt_pig_vcfs
num_snp_escape = vector(mode = 'numeric', length = length(filt_pig_vcfs_noEscape))
for(i in 1:length(filt_pig_vcfs_noEscape)){
  
  if(is.null(dim(filt_pig_vcfs_noEscape[[i]]))){next}
  escape_index = filt_pig_vcfs_noEscape[[i]]$gene %in% escape_genes
  filt_pig_vcfs_noEscape[[i]] = filt_pig_vcfs_noEscape[[i]][!escape_index, ]
  num_snp_escape[i] = dim(filt_pig_vcfs_noEscape[[i]])
}


save(filt_pig_vcfs_noEscape, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/sus_scrofa/max_snp_vcf_noEscape_3_15_23.Rdata')

```

```{r}
load(file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/sus_scrofa/max_snp_vcf_noEscape_3_15_23.Rdata')
filt_pig_vcfs_noEscape[1:5]
```


```{r}
Ns.escape = vector(mode='numeric', length = length(filt_pig_vcfs_noEscape))

for(i in 1:length(filt_pig_vcfs_noEscape)){
  
  suppressWarnings(if(is.null(dim(filt_pig_vcfs_noEscape[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_pig_vcfs_noEscape[[i]])[1]})
...
for(i in 1: length(filt_pig_vcfs_noEscape)){
  ...
  total_exp = filt_pig_vcfs_noEscape[[i]]$ref_counts + filt_pig_vcfs_noEscape[[i]]$alt_counts
  ref_ratios = filt_pig_vcfs_noEscape[[i]]$ref_counts / total_exp
  ratios.escape.max[[i]] = ref_ratios
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_pig_vcfs_noEscape), function(i) 
  est_skew_func(filt_pig_vcfs_noEscape[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
num_good_snps_escape = vector(length = length(filt_pig_vcfs_noEscape), mode = 'numeric')

for(i in 1:length(filt_pig_vcfs_noEscape)){
  
  data = filt_pig_vcfs_noEscape[[i]]
...
length(escape_genes)
sample_index = which(est_skew_no_escape >= .7 & num_good_snps_escape >= 10)

for(i in 1:length(sample_index)){
  
  data = filt_pig_vcfs_noEscape[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(sample_index)){
  ...
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  hist(ratios, breaks = seq(0, 1, .025))
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/sus_scrofa/bootstrapping_results_3_15_23.Rdata')
length(boots)
boots[1:5]
```
...
```
```{r}
length(num_good_snps_escape)
```


```{r}

save(pig_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/sus_scrofa/pig_skew_and_stats_df_3_15_23.Rdata')

```


```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/sus_scrofa/pig_skew_and_stats_df_3_15_23.Rdata')
...
sum(pig_skew_and_stats_df$num_good_snps_no_escape >= 10)
hist(pig_skew_and_stats_df$est_skew_no_escape[pig_skew_and_stats_df$num_good_snps_no_escape >= 10], breaks = seq(.5,1, .005))
...
quantile( pig_skew_and_stats_df$CI_width, probs = seq(0,1,.05), na.rm = T)
quantile( pig_skew_and_stats_df$est_var_no_escape, probs = seq(0,1,.05), na.rm = T)

sample_index = pig_skew_and_stats_df$num_good_snps_no_escape >= 10 & pig_skew_and_stats_df$CI_width < .1458 & pig_skew_and_stats_df$est_var_no_escape < .12
sum(sample_index)
hist(pig_skew_and_stats_df$est_skew_no_escape, breaks = seq(.5,1, .005))
hist(pig_skew_and_stats_df$est_skew_no_escape[sample_index], breaks = seq(.5,1, .005))

hist(unfold(pig_skew_and_stats_df$est_skew_no_escape[sample_index]), breaks = seq(0,1, .01))
...
sample_index = which(pig_skew_and_stats_df$num_good_snps_no_escape >= 10 & pig_skew_and_stats_df$CI_width >= .1458 & pig_skew_and_stats_df$est_var_no_escape >= .12)
...
data = filt_pig_vcfs_noEscape[[sample_index[7]]]
data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
ratios
data
hist(ratios, breaks = seq(0,1,.025), main = est_skew_no_escape[sample_index[7]])
```

```{r}

sample_index = pig_skew_and_stats_df$num_good_snps_no_escape >= 10
sum(sample_index)
hist(pig_skew_and_stats_df$est_skew_no_escape, breaks = seq(.5,1, .005))
hist(pig_skew_and_stats_df$est_skew_no_escape[sample_index], breaks = seq(.5,1, .005))
...
```{r}
num_samples = length(filt_pig_vcfs)
...
ggplot(snp_stat_df[snp_filter, ], aes(x = snp_pos, y = prop_samples_present)) + geom_point(alpha = .5, size = .5) + xlab('X chromosome coordinate') + 
  ylab('% of samples SNP is present and passed QC') + ggtitle(sprintf('Het SNPs (%i) present in Sus scrofa bulk RNA-seq samples (%i) ',num_snps_present, num_samples))


ggplot(snp_stat_df[snp_filter, ], aes(x = snp_pos, y = mean_total_counts)) + geom_point(alpha = .5, size = .5) + xlab('X chromosome coordinate') + 
  ylab('mean total reads') + ggtitle(sprintf('Het SNPs (%i) present in Sus scrofa bulk RNA-seq samples (%i) ',num_snps_present, num_samples))
...
p1 = ggplot(pig_skew_and_stats_df, aes(x = est_skew, y = num_good_snps)) + geom_point(alpha = .5, size = .5) + xlab('Estimated XCI skew') + ylab("Number of SNPs skew estimate is derived from") + ggtitle('Sus scrofa samples') + xlim(.5,1)
