...
'/grid/gillis/data/hover/work/cmxci1.paired/Canis_lupus_familiaris'
'/grid/gillis/data/hover/work/cmxci1.single/Canis_lupus_familiaris'
...
annot_dog_vcfs = wasp_filt_vcf

for(j in 1:length(annot_dog_vcfs)){
  test = annot_dog_vcfs[[j]]
  ...
  for( i in 1:dim(test)[1]){
    ...
    if(test_gene_start <= x_gene_ends[gene_index]){
      ...
      gene_annots[i] = dog_x_meta$gene[gene_index]
      gene_biotype[i] = dog_x_meta$gene_biotype[gene_index]
    }
  }
  
  test$gene = gene_annots
  test$gene_biotype = gene_biotype
  annot_dog_vcfs[[j]] = test
}
...
```{r}
save(annot_dog_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/all_snp_vcf_3_15_23.Rdata')
...
captured_genes = c()
for( i in 1:length(annot_dog_vcfs)){
  suppressWarnings(if(is.null(dim(annot_dog_vcfs[[i]]))){next})
  captured_genes = c(captured_genes, annot_dog_vcfs[[i]]$gene)
}
...
for(i in 1:100){
  ...
  gene_ref_exp = c()
  for(j in 1:length(annot_dog_vcfs)){
    suppressWarnings(if(is.null(dim(annot_dog_vcfs[[j]]))){next})
    gene_index = annot_dog_vcfs[[j]]$gene == current_gene
    ref_counts = annot_dog_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_dog_vcfs[[j]]$alt_counts[gene_index]
...
for( i in 1:length(annot_dog_vcfs)){
  suppressWarnings(if(is.null(dim(annot_dog_vcfs[[i]]))){next})
  captured_SNPs = c(captured_SNPs, annot_dog_vcfs[[i]]$Pos)
}
...
for(i in 1:length(captured_SNPs)){
  ...
  for(j in 1:length(annot_dog_vcfs)){
    suppressWarnings(if(is.null(dim(annot_dog_vcfs[[j]]))){next})
    gene_index = annot_dog_vcfs[[j]]$Pos == current_SNP
    ref_counts = annot_dog_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_dog_vcfs[[j]]$alt_counts[gene_index]
    total_counts = ref_counts + alt_counts
...
```{r}
hist(snp_stat_df$mean_ref_ratio, breaks = seq(0,1,.01), xlab = 'Mean reference ratio per SNP', main = 'Canis lupus familiaris')
...
```{r}
save(snp_stat_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/detected_het_snps_df_3_15_23.Rdata')

dog_snp_stat_df = snp_stat_df
save(dog_snp_stat_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/dog_snp_stat_df.Rdata')

```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/detected_het_snps_df_3_15_23.Rdata')
snp_stat_df
```
...
#ggsave(plot = p3, filename = 'dog_x_ref_snp_filtering_plots.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/ref_snp_filtering/', device = 'pdf',
...
```{r}
filt_dog_vcfs = annot_dog_vcfs

for(i in 1:length(filt_dog_vcfs)){
  test = filt_dog_vcfs[[i]]
  
  suppressWarnings(if(is.null(dim(test))){next})
  
  #Get rid of SNPs in nonannotated genes
  test = test[test$gene != "", ]
  suppressWarnings(if(dim(test)[1] == 0){filt_dog_vcfs[[i]] = NA; next})
...
save(filt_dog_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/max_snp_vcf_3_15_23.Rdata')


```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/max_snp_vcf_3_15_23.Rdata')

```



look at distribution of total read counts for the SNPs that have reference ratios about .5. A lot of these could be lowly expressed
...
for(i in 1:length(filt_dog_vcfs)){
  ...
  if(is.null(dim(filt_dog_vcfs[[i]]))){next}
  data = filt_dog_vcfs[[i]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  total_counts = data$ref_counts + data$alt_counts
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  balanced_filter = ref_ratios < 0.6 & ref_ratios > 0.4
  
  all_total_counts = c(all_total_counts, total_counts[balanced_filter])
  all_ref_ratios = c(all_ref_ratios, ref_ratios[balanced_filter])

}

plot(all_ref_ratios, log10(all_total_counts))
...
num_filt_genes = vector(mode = 'numeric', length = length(filt_dog_vcfs))
for(i in 1:length(filt_dog_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_dog_vcfs[[i]]))){
    num_filt_genes[i] = 0
    next
    
  })
  
  num_filt_genes[i] = dim(filt_dog_vcfs[[i]])[1]
...
index_num_samps = which(num_filt_genes >= 10)
...
for(i in index){
  ...
  data = filt_dog_vcfs[[index_num_samps[i]]]
...
```{r}
Ns = vector(mode='numeric', length = length(filt_dog_vcfs))

for(i in 1:length(filt_dog_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_dog_vcfs[[i]]))){Ns[i] = 0}
  else{Ns[i] = dim(filt_dog_vcfs[[i]])[1]})
...
for(i in 1: length(filt_dog_vcfs)){
  ...
  total_exp = filt_dog_vcfs[[i]]$ref_counts + filt_dog_vcfs[[i]]$alt_counts
  ref_ratios = filt_dog_vcfs[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits = mclapply(1:length(filt_dog_vcfs), function(i) est_skew_func(filt_dog_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()


Sys.time()
folded_norm_fits_bad_snps = mclapply(1:length(filt_dog_vcfs), function(i) est_skew_func(filt_dog_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df, filter_snps = F), mc.cores = 20)
Sys.time()
...
hist(est_skew, main = 'dog skew estimates filtering out bad SNPs', breaks=64)
...
num_good_snps = vector(length = length(filt_dog_vcfs), mode = 'numeric')
total_snps = vector(length = length(filt_dog_vcfs), mode = 'numeric')
...
```{r}
par(pty = 's')
plot(est_skew, num_good_snps, cex = .5)
abline(h = 10, col = 'red')


plot(est_skew_bad_snps, total_snps, cex = .5)
abline(h = 10, col = 'red')
```
...
all_ref_ratios = c()
all_snp_pos = c()
dog_het_index = which(est_skew >= .7)
for(i in 1:length(dog_het_index)){

  data = annot_dog_vcfs[[dog_het_index[i]]]
  if(is.null(dim(data))){next}
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts /(data$ref_counts + data$alt_counts)
...
bad_dog_bins = which(mean_bin_ref_ratio<= .675)
bad_dog_bins = c(c(1,2,3,4,5,6,7), bad_dog_bins)
bad_dog_bins = unique(bad_dog_bins)


binned_chrom_df$keep_label = rep('Kept 1MB bin', length = nrow(binned_chrom_df))
binned_chrom_df$keep_label[(binned_chrom_df$bin/1e6) %in% (bad_dog_bins - 1)] = 'Excluded 1MB bin'
p1 = ggplot(binned_chrom_df, aes(x = bin, y = mean_bin_ref_ratio, size = num_snps, color = keep_label)) + geom_point(alpha = .5) + geom_hline(yintercept = .675, color = 'red') +
  ylab('Mean folded SNP ratio per bin') + xlab('X chromosome position (1MB bins)') + ggtitle('dog All SNPs')+ ylim(.5, 1) +
  scale_color_manual(values = c('Kept 1MB bin' = 'black', 'Excluded 1MB bin' = 'red'))  
ggMarginal(p1, margins = 'y', type = 'histogram', yparams = list(binwidth = .005))
#ggsave(filename = 'dog_binnedMB_escape_scatter.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/escape/', device = 'pdf', 
#       useDingbats = F, height = 4, width = 6)

dog_binned_chrom_df = binned_chrom_df
save(dog_binned_chrom_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/dog_binned_chrom_df.Rdata')
...
bad_dog_coordinates = unlist(lapply(1:length(bad_dog_bins), function(i) ((bad_dog_bins - 1) *1e6)[i]:((bad_dog_bins)*1e6)[i]))
...
filter_bad_bins = function(data){
  if(is.null(dim(data))){return(NA)}
    ...
    escape_index = data$Pos %in% bad_dog_coordinates
    data = data[!escape_index, ]
    return(data)
  }
}

filt_dog_vcfs_noBadbins = mclapply(filt_dog_vcfs, filter_bad_bins, mc.cores = 10)
save(filt_dog_vcfs_noBadbins, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/max_snp_vcf_binFilt_4_27_23.Rdata')
```
...
num_snp_good_bins = vector(mode = 'numeric', length = length(filt_dog_vcfs_noBadbins))
for(i in 1:length(filt_dog_vcfs_noBadbins)){
  
  data = filt_dog_vcfs_noBadbins[[i]]
...
```{r}
Ns.escape = vector(mode='numeric', length = length(filt_dog_vcfs_noBadbins))

for(i in 1:length(filt_dog_vcfs_noBadbins)){
  
  suppressWarnings(if(is.null(dim(filt_dog_vcfs_noBadbins[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_dog_vcfs_noBadbins[[i]])[1]})
...
for(i in 1: length(filt_dog_vcfs_noBadbins)){
  ...
  total_exp = filt_dog_vcfs_noBadbins[[i]]$ref_counts + filt_dog_vcfs_noBadbins[[i]]$alt_counts
  ref_ratios = filt_dog_vcfs_noBadbins[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_dog_vcfs_noBadbins), function(i) 
  est_skew_func(filt_dog_vcfs_noBadbins[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
for(i in 1:length(sample_index)){
  ...
  data = filt_dog_vcfs_noBadbins[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
escape_mean = rep(NA, length = length(annot_dog_vcfs))
escape_var = rep(NA, length = length(annot_dog_vcfs))
num_escape_snps = rep(NA, length = length(annot_dog_vcfs))
inact_mean = rep(NA, length = length(annot_dog_vcfs))
inact_var = rep(NA, length = length(annot_dog_vcfs))
num_inact_snps = rep(NA, length = length(annot_dog_vcfs))

for(i in 1:length(annot_dog_vcfs)){

  data = annot_dog_vcfs[[i]]
  if(is.null(dim(data))){next}
  escape_index = (plyr::round_any(data$Pos, 1000000, ceiling)/1e6) %in% bad_dog_bins #Get the SNPs located in the escape bins
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(maybe_ref_biased)){

  data = filt_dog_vcfs_noBadbins[[maybe_ref_biased[i]]]
  if(is.null(dim(data))){next}
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  hist(ref_ratios, breaks = seq(0,1,.025), main = est_skew_no_escape[maybe_ref_biased[i]])
...
for(i in 1:length(chrX_skewed_index)){
  ...
  data = filt_dog_vcfs_noBadbins[[chrX_skewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(chrX_Nonskewed_index)){
  ...
  data = filt_dog_vcfs_noBadbins[[chrX_Nonskewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(enoughSNPs_index)){
  ...
  data = filt_dog_vcfs_noBadbins[[enoughSNPs_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  all_ref_ratios = c(all_ref_ratios, ref_ratios)
...
ggplot(skew_bin_snp_df, aes(x = ref_ratios, y = bin_label)) + geom_density_ridges(rel_min_height = 0.01, scale = 0.95, fill = species_colors[10] ) + xlim(0,1) +
...
#ggsave(filename = 'dog_chrX_skew_binned_refRatio_ridgePlot.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/species_skew_binned_refRatios/', 
#       device = 'pdf', useDingbats = F, height = 6, width = 6)

dog_skew_bin_snp_df = skew_bin_snp_df
save(dog_skew_bin_snp_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/dog_skew_bin_snp_df.Rdata')
...
```{r}
dog_skew_and_stats_df = data.frame(sample_id = wig_samples, 
...
dog_skew_and_stats_df
```


```{r}

save(dog_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/dog_skew_and_stats_binFilt_df_4_27_23.Rdata')
...
all_ref_ratios = c()
all_snp_pos = c()
dog_het_index = which(est_skew_no_escape >= .7)
for(i in 1:length(dog_het_index)){

  data = filt_dog_vcfs_noBadbins[[dog_het_index[i]]]
  if(is.null(dim(data))){next}
  #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts /(data$ref_counts + data$alt_counts)
...
for(i in 1:length(filt_dog_vcfs)){
  if(is.null(dim(filt_dog_vcfs[[i]]))){next}
  all_genes = c(all_genes,filt_dog_vcfs[[i]]$gene )
}
...
for(j in 1:length(all_genes)){
  ...
  for(i in 1:length(skewed_index)){
    ...
    if(is.null(dim(filt_dog_vcfs[[skewed_index[i]]]))){next}
    
    data = filt_dog_vcfs[[skewed_index[i]]]
    #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
sample_index = which(total_snps >= 10 & est_skew >= .6)
all_escape_ratios = c()
all_non_ratios  = c()
for(i in 1:length(sample_index)){
  
  data = filt_dog_vcfs[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
hist(all_escape_ratios, breaks = seq(0,1,.025), col = 'red')
hist(all_non_ratios, add = T, breaks = seq(0,1,.025))
...
escape_dog_df = data.frame(gene = all_genes, num_pvalues = num_pvalues,
...
save(escape_dog_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/escape_dog_df_3_15_23.Rdata')
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/escape_dog_df_3_15_23.Rdata')
dog_escape = escape_dog_df$gene[escape_dog_df$escape_label == 'escape']

conserved_escape = intersect(dog_escape, sheep_escape)
...
escape_genes = escape_dog_df$gene[escape_dog_df$escape_label == 'escape']
...
for(i in 1:nrow(snp_stat_df)){
  gene_index = findInterval(snp_stat_df$snp_pos[i], dog_x_meta$start)
  if(gene_index == 0){gene_name_vec[i] = NA; next}
  
  if(snp_stat_df$snp_pos[i] <= dog_x_meta$end[gene_index]){
    gene_name_vec[i] = dog_x_meta$gene[gene_index]
...
gene_pos_df = data.frame(gene_name = dog_x_meta$gene, gene_start = dog_x_meta$start, gene_end = dog_x_meta$end, 
                         gene_biotype = dog_x_meta$gene_biotype, escape_label = dog_x_meta$gene %in% escape_genes)
...
```{r}
filt_dog_vcfs_noEscape = filt_dog_vcfs
num_snp_escape = vector(mode = 'numeric', length = length(filt_dog_vcfs_noEscape))
for(i in 1:length(filt_dog_vcfs_noEscape)){
  
  if(is.null(dim(filt_dog_vcfs_noEscape[[i]]))){next}
  escape_index = filt_dog_vcfs_noEscape[[i]]$gene %in% escape_genes
  filt_dog_vcfs_noEscape[[i]] = filt_dog_vcfs_noEscape[[i]][!escape_index, ]
  num_snp_escape[i] = dim(filt_dog_vcfs_noEscape[[i]])
}


save(filt_dog_vcfs_noEscape, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/max_snp_vcf_noEscape_3_15_23.Rdata')

```

```{r}
load(file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/max_snp_vcf_noEscape_3_15_23.Rdata')
filt_dog_vcfs_noEscape[1:5]
```


```{r}
Ns.escape = vector(mode='numeric', length = length(filt_dog_vcfs_noEscape))

for(i in 1:length(filt_dog_vcfs_noEscape)){
  
  suppressWarnings(if(is.null(dim(filt_dog_vcfs_noEscape[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_dog_vcfs_noEscape[[i]])[1]})
...
for(i in 1: length(filt_dog_vcfs_noEscape)){
  ...
  total_exp = filt_dog_vcfs_noEscape[[i]]$ref_counts + filt_dog_vcfs_noEscape[[i]]$alt_counts
  ref_ratios = filt_dog_vcfs_noEscape[[i]]$ref_counts / total_exp
  ratios.escape.max[[i]] = ref_ratios
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_dog_vcfs_noEscape), function(i) 
  est_skew_func(filt_dog_vcfs_noEscape[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
num_good_snps_escape = vector(length = length(filt_dog_vcfs_noEscape), mode = 'numeric')

for(i in 1:length(filt_dog_vcfs_noEscape)){
  
  data = filt_dog_vcfs_noEscape[[i]]
...
```{r}
1/100
length(est_skew_no_escape[num_good_snps_escape >= 10])
hist(unfold(est_skew_no_escape[num_good_snps_escape >= 10]), breaks = seq(0, 1, .01))
hist(est_skew[num_good_snps >= 10], breaks = seq(.5, 1, .01))

par(pty = 's')
plot(est_skew, est_skew_no_escape, xlim = c(.5, 1), ylim = c(.5, 1))
abline(a = 0, b = 1, col = 'red')
```
filtering out escape and looking at the higher read depth genes



```{r}
Ns_higher_escape = vector(mode='numeric', length = length(filt_dog_vcfs_noEscape))

for(i in 1:length(filt_dog_vcfs_noEscape)){
  data = filt_dog_vcfs_noEscape[[i]]
  if(is.null(dim(data))){Ns_higher_escape[i] = 0}
  else{
    
    total_counts = data$ref_counts + data$alt_counts
    data = data[total_counts >= 50, ]
    Ns_higher_escape[i] = dim(data)[1]
    
    }
  
}

Ns_higher_escape[1:10]
```

```{r}

ratios.max_higher_escape = list()
for(i in 1: length(filt_dog_vcfs_noEscape)){
  
  if(Ns_higher_escape[i] == 0){next}
  data = filt_dog_vcfs_noEscape[[i]]
  total_exp = data$ref_counts + data$alt_counts
  exp_filt = total_exp >= 50
  ref_ratios = data$ref_counts[exp_filt] / total_exp[exp_filt]
  ratios.max_higher_escape[[i]] = ref_ratios

}

head(ratios.max_higher_escape)
...
Sys.time()
folded_norm_fits_higher_escape = mclapply(1:length(filt_dog_vcfs_noEscape), function(i) est_skew_func_higher(list.skew.max = filt_dog_vcfs_noEscape[[i]], Ns = Ns_higher_escape[i], 
                                                                                             ratios.max = ratios.max_higher_escape[[i]], snp_stat_df = snp_stat_df, filter_snps = T), mc.cores = 20)
Sys.time()

#Grab the skew and variance estimates
est_skew_higher_escape = vector(mode = 'numeric', length = length(folded_norm_fits_higher_escape))
est_var_higher_escape = vector(mode = 'numeric', length = length(folded_norm_fits_higher_escape))
for(i in 1:length(folded_norm_fits_higher_escape)){
  
  if(is.na(folded_norm_fits_higher_escape[i])){
    est_skew_higher_escape[i] = NA
    est_var_higher_escape[i] = NA
    next
  }
  
  est_skew_higher_escape[i] = folded_norm_fits_higher_escape[[i]][[1]]
  est_var_higher_escape[i] = folded_norm_fits_higher_escape[[i]][[2]]
}



hist(unfold(est_skew_higher_escape[Ns_higher_escape >=10]), breaks = seq(0, 1, .01))
par(pty = 's')
plot(est_skew, est_skew_higher_escape)
abline(a = 0, b = 1, col = 'red')
plot(est_skew_higher_escape, Ns_higher_escape)
abline(h = 10, col = 'red')
...
```{r}
quantile(est_var_no_escape, probs = seq(0,1,.05), na.rm = T)
sample_index = which(num_good_snps_escape >=10 & est_skew_no_escape <=.8 & est_skew_no_escape >= .7  )

length(sample_index)
all_ratios = c()
all_maybe_escape_genes = c()
hist(unfold(est_skew_no_escape[sample_index]), breaks = seq(0,1,.005))

for(i in 1:length(sample_index)){

  data = filt_dog_vcfs_noEscape[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ...
  maybe_escape_index = ref_ratios <= .6 & ref_ratios >= .4
  maybe_escape_genes = data$gene[maybe_escape_index]
  all_maybe_escape_genes = c(all_maybe_escape_genes, maybe_escape_genes)
  
  all_ratios = c(all_ratios, ref_ratios)
  hist(ref_ratios, xlim = c(0,1), breaks = seq(0,1, .025), main = sprintf('%0.3f : sample index %i',est_skew_no_escape[sample_index[i]], sample_index[i]))
  abline(v = .5, col = 'red')

}

hist(all_ratios, breaks = seq(0,1,.025))
all_maybe_escape_genes = unique(all_maybe_escape_genes)
```
...
for(i in 1:length(sample_index)){
  ...
  all_ref_ratios = c(all_ref_ratios, ref_ratios)
  all_total_counts = c(all_total_counts, (data$ref_counts+data$alt_counts) )
}


plot(all_ref_ratios, log10(all_total_counts))
hist(all_ref_ratios, breaks = seq(0,1,.025))
hist(log10(all_total_counts))
```
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/bootstrapping_results_readFilt_diff_3_15_23.Rdata')
length(boots)
boots[1:5]
```
...
save(dog_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/dog_skew_and_stats_df_readFilt_diff_3_15_23.Rdata')

```

```{r}

load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/dog_skew_and_stats_df_3_15_23.Rdata')
...
```{r}
quantile(dog_skew_and_stats_df$CI_width, probs = seq(0,1,.05), na.rm = T)
quantile(dog_skew_and_stats_df$est_var_no_escape, probs = seq(0,1,.05), na.rm = T)

sample_index = which(dog_skew_and_stats_df$num_good_snps_no_escape >= 10 & dog_skew_and_stats_df$CI_width < .2715 & dog_skew_and_stats_df$est_var_no_escape < .162 )

length(sample_index)

hist(unfold(dog_skew_and_stats_df$est_skew_no_escape[sample_index]), breaks = seq(0,1,.01), main = 'Dog population XCI after 1st escape filtering')
...
for(i in 1:length(sample_index)){
  ...
  hist(ref_ratios, xlim = c(0,1), breaks = seq(0,1, .025), main = dog_skew_and_stats_df$est_skew_no_escape[sample_index[i]])
  abline(v = .5, col = 'red')
...
second round of finding escape genes
...
for(i in 1:length(filt_dog_vcfs)){
  if(is.null(dim(filt_dog_vcfs_noEscape[[i]]))){next}
  all_genes = c(all_genes,filt_dog_vcfs_noEscape[[i]]$gene )
}
...
names(gene_chi_left_p_values) = all_genes
num_pvalues =  vector(mode = 'numeric', length = length(all_genes))
skewed_index = which(est_skew_no_escape >= .6)
...
for(j in 1:length(all_genes)){
  ...
  for(i in 1:length(skewed_index)){
    ...
    if(is.null(dim(filt_dog_vcfs_noEscape[[skewed_index[i]]]))){next}
    
    data = filt_dog_vcfs_noEscape[[skewed_index[i]]]
    #data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
...
new_escape_genes = all_genes[escape_gene_index]
sample_index = which(est_skew_no_escape >= .6)
...
for(i in 1:length(sample_index)){
  ...
  if(is.null(dim(data))){next}
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  escape_present = data$gene %in% new_escape_genes
  
  ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  
  #hist(ratios[!escape_present], breaks = seq(0,1,.025))
  #hist(ratios[escape_present], add = T, col = 'red', breaks = seq(0,1,.025))

  non_escape_ratios = c(non_escape_ratios,ratios[!escape_present] )
  escape_ratios = c(escape_ratios,ratios[escape_present] )
}

hist(non_escape_ratios, breaks = seq(0,1,.025))
hist(escape_ratios, breaks = seq(0,1,.025), add = T, col = 'red')
...
```{r}
filt_dog_vcfs_noEscape_2 = filt_dog_vcfs_noEscape
num_snp_escape = vector(mode = 'numeric', length = length(filt_dog_vcfs_noEscape_2))
for(i in 1:length(filt_dog_vcfs_noEscape_2)){
  
  if(is.null(dim(filt_dog_vcfs_noEscape_2[[i]]))){next}
  escape_index = filt_dog_vcfs_noEscape_2[[i]]$gene %in% new_escape_genes
  filt_dog_vcfs_noEscape_2[[i]] = filt_dog_vcfs_noEscape_2[[i]][!escape_index, ]
  num_snp_escape[i] = dim(filt_dog_vcfs_noEscape_2[[i]])
}

save(filt_dog_vcfs_noEscape_2, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/max_snp_vcf_noEscape_round_2_3_15_23.Rdata')
```



```{r}
Ns.escape = vector(mode='numeric', length = length(filt_dog_vcfs_noEscape_2))

for(i in 1:length(filt_dog_vcfs_noEscape_2)){
  
  suppressWarnings(if(is.null(dim(filt_dog_vcfs_noEscape_2[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_dog_vcfs_noEscape_2[[i]])[1]})
...
for(i in 1: length(filt_dog_vcfs_noEscape_2)){
  ...
  total_exp = filt_dog_vcfs_noEscape_2[[i]]$ref_counts + filt_dog_vcfs_noEscape_2[[i]]$alt_counts
  ref_ratios = filt_dog_vcfs_noEscape_2[[i]]$ref_counts / total_exp
  ratios.escape.max[[i]] = ref_ratios
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_dog_vcfs_noEscape_2), function(i) 
  est_skew_func(filt_dog_vcfs_noEscape_2[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()

#Grab the skew and variance estimates
est_skew_no_escape_2 = vector(mode = 'numeric', length = length(folded_norm_fits_no_escape))
est_var_no_escape_2 = vector(mode = 'numeric', length = length(folded_norm_fits_no_escape))
for(i in 1:length(folded_norm_fits)){
  
  if(is.na(folded_norm_fits_no_escape[i])){
    est_skew_no_escape_2[i] = NA
    est_var_no_escape_2[i] = NA
    next
  }
  
  est_skew_no_escape_2[i] = folded_norm_fits_no_escape[[i]][[1]]
  est_var_no_escape_2[i] = folded_norm_fits_no_escape[[i]][[2]]
}
...
num_good_snps_escape = vector(length = length(filt_dog_vcfs_noEscape_2), mode = 'numeric')

for(i in 1:length(filt_dog_vcfs_noEscape_2)){
  
  data = filt_dog_vcfs_noEscape_2[[i]]
  suppressWarnings(if(is.null(dim(data))){num_good_snps_escape[i] = 0; next})
...
```{r}
hist(est_skew_no_escape_2[num_good_snps_escape >= 10], breaks = seq(.5, 1, .005))
hist(est_skew[num_good_snps >= 10], breaks = seq(.5, 1, .005))

par(pty = 's')
plot(est_skew, est_skew_no_escape_2, xlim = c(.5, 1), ylim = c(.5, 1))
abline(a = 0, b = 1, col = 'red')

par(pty = 's')
plot(est_skew_no_escape, est_skew_no_escape_2, xlim = c(.5, 1), ylim = c(.5, 1))
abline(a = 0, b = 1, col = 'red')
```
Look at samples
...
sample_index = which(num_good_snps_escape >=10 & est_skew_no_escape_2 >= .75  )

length(sample_index)
all_ratios = c()

hist(unfold(est_skew_no_escape_2[sample_index]), breaks = seq(0,1,.05))

for(i in 1:length(sample_index)){

  data = filt_dog_vcfs_noEscape_2[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ...
  all_ratios = c(all_ratios, ref_ratios)
  hist(ref_ratios, xlim = c(0,1), breaks = seq(0,1, .025), main = est_skew_no_escape_2[sample_index[i]])
  abline(v = .5, col = 'red')
...
hist(unfold(est_skew_no_escape_2[num_good_snps_escape >= 10]), breaks = seq(0,1,1/100), main = 'Dog population XCI after 2nd escape filtering')
hist(unfold(est_skew_no_escape_2[num_good_snps_escape >= 10]), breaks = seq(0,1,1/100), main = 'Dog population XCI after 2nd escape filtering', freq = F)
...
save(escape_dog_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/escape_dog_round_2_df_3_15_23.Rdata')
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/bootstrapping_results_escape_round_2.Rdata')
length(boots)
boots[1:5]
```
...
dog_skew_and_stats_df = data.frame(sample_id = wig_samples, 
                                   est_skew = est_skew, est_var = est_var,  #Skew estimates filtering out bad genes
                                   est_skew_total_snps = est_skew_bad_snps, est_var_total_snps = est_var_bad_snps,   #Skew estimates with all genes 
                                   est_skew_no_escape = est_skew_no_escape_2, est_var_no_escape = est_var_no_escape_2,   #Skew estimates filtering out escape
                                   num_good_snps = num_good_snps, total_snps = total_snps, num_good_snps_no_escape = num_good_snps_escape,
                                   lower_CI = lower_CI, upper_CI = upper_CI, CI_width = CI_width)
...
save(dog_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/canis_lupus_familiaris/dog_skew_and_stats_df_escape_round_2_3_15_23.Rdata')
