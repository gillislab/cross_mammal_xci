...
/grid/gillis/data/hover/work/cmxci.paired/Macaca_mulatta
/grid/gillis/data/hover/work/cmxci.single/Macaca_mulatta
...
processed_files = list.files('/grid/gillis/home/hover/work/cmxci.paired/Macaca_mulatta.X')
...
processed_files = list.files('/grid/gillis/home/hover/work/cmxci.single/Macaca_mulatta.X')
...
wasp_vcf_files_paired = paste('/grid/gillis/home/hover/work/cmxci.paired/Macaca_mulatta.X/',wasp_vcf_files_paired, sep = '')
wasp_vcf_files_single = paste('/grid/gillis/home/hover/work/cmxci.single/Macaca_mulatta.X/',wasp_vcf_files_single, sep = '')

wasp_wig_files_paired = paste('/grid/gillis/home/hover/work/cmxci.paired/Macaca_mulatta.X/',wasp_wig_files_paired, sep = '')
wasp_wig_files_single = paste('/grid/gillis/home/hover/work/cmxci.single/Macaca_mulatta.X/',wasp_wig_files_single, sep = '')
...
annot_macaca_vcfs = wasp_filt_vcf

for(j in 1:length(annot_macaca_vcfs)){
  test = annot_macaca_vcfs[[j]]
  ...
  for( i in 1:dim(test)[1]){
    ...
    gene_index = findInterval(test_gene_start, x_gene_starts)#Finds the index where the SNP is between two gene starts
    if(gene_index == 0){next}
    
    #If it's in the gene body, add the annotations
    if(test_gene_start <= x_gene_ends[gene_index]){
      
      gene_annots[i] = macaca_x_meta$gene[gene_index]
      gene_biotype[i] = macaca_x_meta$gene_biotype[gene_index]
    }
  }
  
  test$gene = gene_annots
  test$gene_biotype = gene_biotype
  annot_macaca_vcfs[[j]] = test
}

head(annot_macaca_vcfs)
...
```{r}
save(annot_macaca_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/all_snp_vcf_3_15_23.Rdata')
...
captured_genes = c()
for( i in 1:length(annot_macaca_vcfs)){
  suppressWarnings(if(is.null(dim(annot_macaca_vcfs[[i]]))){next})
  captured_genes = c(captured_genes, annot_macaca_vcfs[[i]]$gene)
}
...
for(i in 1:100){
  ...
  gene_ref_exp = c()
  for(j in 1:length(annot_macaca_vcfs)){
    suppressWarnings(if(is.null(dim(annot_macaca_vcfs[[j]]))){next})
    gene_index = annot_macaca_vcfs[[j]]$gene == current_gene
    ref_counts = annot_macaca_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_macaca_vcfs[[j]]$alt_counts[gene_index]
...
for( i in 1:length(annot_macaca_vcfs)){
  suppressWarnings(if(is.null(dim(annot_macaca_vcfs[[i]]))){next})
  captured_SNPs = c(captured_SNPs, annot_macaca_vcfs[[i]]$Pos)
}
...
for(i in 1:length(captured_SNPs)){
  ...
  for(j in 1:length(annot_macaca_vcfs)){
    suppressWarnings(if(is.null(dim(annot_macaca_vcfs[[j]]))){next})
    gene_index = annot_macaca_vcfs[[j]]$Pos == current_SNP
    ref_counts = annot_macaca_vcfs[[j]]$ref_counts[gene_index]
    alt_counts = annot_macaca_vcfs[[j]]$alt_counts[gene_index]
    total_counts = ref_counts + alt_counts
...
hist(snp_stat_df$mean_ref_ratio, breaks = seq(0,1,.01), xlab = 'Mean reference ratio per SNP', main = 'Macaca mulatta')
hist(snp_stat_df$num_samples_present)
par(pty = 's')
plot(snp_stat_df$mean_ref_ratio,snp_stat_df$num_samples_present, cex = .25, xlab = 'Mean reference ratio', 
     ylab = '# of samples SNP is detected', main = 'Macaca mulatta Het SNPs')
...
Filter out SNPs present in at least 10 samples that have a mean reference ratio < .4 or > .6, 
...
hist(snp_stat_df$mean_ref_ratio[snp_stat_df$filter], breaks = seq(0,1,.01))
plot(snp_stat_df$num_samples_present,snp_stat_df$mean_ref_ratio, cex = .25)
plot(snp_stat_df$num_samples_present[snp_stat_df$filter],snp_stat_df$mean_ref_ratio[snp_stat_df$filter], cex = .25)
...
save(snp_stat_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/detected_het_snps_df_3_15_23.Rdata')

macaca_snp_stat_df = snp_stat_df
save(macaca_snp_stat_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/macaca_snp_stat_df.Rdata')
```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/detected_het_snps_df_3_15_23.Rdata')
...
#ggsave(plot = p3, filename = 'macaca_x_ref_snp_filtering_plots.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/ref_snp_filtering/', device = 'pdf',
...
num_tot_vec = vector(mode = 'numeric', length = length(annot_macaca_vcfs) )
num_gene_vec = vector(mode = 'numeric', length = length(annot_macaca_vcfs) )
all_intergene_ratio = c()
all_gene_ratio = c()

for(i in 1:length(annot_macaca_vcfs)){
  
  if(is.null(dim(annot_macaca_vcfs[[i]]))){next}
  data = annot_macaca_vcfs[[i]]
...
```{r}
filt_macaca_vcfs = annot_macaca_vcfs

for(i in 1:length(filt_macaca_vcfs)){
  test = filt_macaca_vcfs[[i]]
  suppressWarnings(if(is.null(dim(test))){next})
  #Get rid of SNPs in nonannotated genes
  test = test[test$gene != "", ]
  suppressWarnings(if(dim(test)[1] == 0){filt_macaca_vcfs[[i]] = NA; next})
...
```{r}
save(filt_macaca_vcfs, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/max_snp_vcf_3_15_23.Rdata')


```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/max_snp_vcf_3_15_23.Rdata')
...
```{r}
num_filt_genes = vector(mode = 'numeric', length = length(filt_macaca_vcfs))
for(i in 1:length(filt_macaca_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_macaca_vcfs[[i]]))){
    num_filt_genes[i] = 0
    next
    
  })
  
  num_filt_genes[i] = sum(filt_macaca_vcfs[[i]]$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter])
...
```{r}
num_total_genes = vector(mode = 'numeric', length = length(annot_macaca_vcfs))
for(i in 1:length(annot_macaca_vcfs)){
  
  suppressWarnings(if(is.null(dim(annot_macaca_vcfs[[i]]))){
    num_total_genes[i] = 0
    next
    
  })
  
  num_total_genes[i] = sum(annot_macaca_vcfs[[i]]$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter])
...
```{r}
par(pty = 's')
plot(num_total_genes, num_filt_genes, ylim = c(0, max(num_total_genes)))
...
for(i in 1:length(index_num_samps)){
  ...
  data = filt_macaca_vcfs[[index_num_samps[i]]]
...
```{r}
Ns = vector(mode='numeric', length = length(filt_macaca_vcfs))

for(i in 1:length(filt_macaca_vcfs)){
  
  suppressWarnings(if(is.null(dim(filt_macaca_vcfs[[i]]))){Ns[i] = 0}
  else{Ns[i] = dim(filt_macaca_vcfs[[i]])[1]})
...
for(i in 1: length(filt_macaca_vcfs)){
  ...
  total_exp = filt_macaca_vcfs[[i]]$ref_counts + filt_macaca_vcfs[[i]]$alt_counts
  ref_ratios = filt_macaca_vcfs[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits = mclapply(1:length(filt_macaca_vcfs), function(i) est_skew_func(filt_macaca_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()


Sys.time()
folded_norm_fits_bad_snps = mclapply(1:length(filt_macaca_vcfs), function(i) est_skew_func(filt_macaca_vcfs[[i]], Ns[i], ratios.max[[i]], snp_stat_df, filter_snps = F), mc.cores = 20)
Sys.time()
...
hist(est_skew, main = 'Macaca skew estimates filtering out bad SNPs', breaks=seq(.5, 1, .005))
hist(est_skew_bad_snps, main = 'Macaca skew estimates filtering out bad SNPs', breaks=seq(.5, 1, .005))
...
num_good_snps = vector(length = length(filt_macaca_vcfs), mode = 'numeric')
total_snps = vector(length = length(filt_macaca_vcfs), mode = 'numeric')

for(i in 1:length(filt_macaca_vcfs)){
  
  data = filt_macaca_vcfs[[i]]
...
all_ref_ratios = c()
all_snp_pos = c()
macaca_het_index = which(est_skew >= .6)
for(i in 1:length(macaca_het_index)){

  data = annot_macaca_vcfs[[macaca_het_index[i]]]
  if(is.null(dim(data))){next}
...
#Excluding the first 5MB as likely pseudo-autosomal regions
#And excluding 1MB bins with a mean reference ratio below .6

bad_macaca_bins = which(mean_bin_ref_ratio<= .6)
bad_macaca_bins = c(c(1,2,3,4,5), bad_macaca_bins)
bad_macaca_bins = unique(bad_macaca_bins)


binned_chrom_df$keep_label = rep('Kept 1MB bin', length = nrow(binned_chrom_df))
binned_chrom_df$keep_label[(binned_chrom_df$bin/1e6) %in% (bad_macaca_bins - 1)] = 'Excluded 1MB bin'
...
ggMarginal(p1, margins = 'y', type = 'histogram', yparams = list(binwidth = .005))
#ggsave(filename = 'macaca_binnedMB_escape_scatter.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/escape/', device = 'pdf', 
#       useDingbats = F, height = 4, width = 6)


macaca_binned_chrom_df = binned_chrom_df
save(macaca_binned_chrom_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/macaca_binned_chrom_df.Rdata')
```
...
bad_macaca_coordinates = unlist(lapply(1:length(bad_macaca_bins), function(i) ((bad_macaca_bins - 1) *1e6)[i]:((bad_macaca_bins)*1e6)[i]))
...
filter_bad_bins = function(data){
  if(is.null(dim(data))){return(NA)}
    ...
    escape_index = data$Pos %in% bad_macaca_coordinates
    data = data[!escape_index, ]
    return(data)
  }
}

filt_macaca_vcfs_noBadbins = mclapply(filt_macaca_vcfs, filter_bad_bins, mc.cores = 10)
save(filt_macaca_vcfs_noBadbins, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/max_snp_vcf_binFilt_4_27_23.Rdata')
```

```{r}
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/max_snp_vcf_binFilt_4_27_23.Rdata')

num_snp_good_bins = vector(mode = 'numeric', length = length(filt_macaca_vcfs_noBadbins))
for(i in 1:length(filt_macaca_vcfs_noBadbins)){
  
  data = filt_macaca_vcfs_noBadbins[[i]]
...
```{r}
Ns.escape = vector(mode='numeric', length = length(filt_macaca_vcfs_noBadbins))

for(i in 1:length(filt_macaca_vcfs_noBadbins)){
  
  suppressWarnings(if(is.null(dim(filt_macaca_vcfs_noBadbins[[i]]))){Ns.escape[i] = 0}
  else{Ns.escape[i] = dim(filt_macaca_vcfs_noBadbins[[i]])[1]})
...
for(i in 1: length(filt_macaca_vcfs_noBadbins)){
  ...
  total_exp = filt_macaca_vcfs_noBadbins[[i]]$ref_counts + filt_macaca_vcfs_noBadbins[[i]]$alt_counts
  ref_ratios = filt_macaca_vcfs_noBadbins[[i]]$ref_counts / total_exp
...
Sys.time()
folded_norm_fits_no_escape = mclapply(1:length(filt_macaca_vcfs_noBadbins), function(i) 
  est_skew_func(filt_macaca_vcfs_noBadbins[[i]], Ns.escape[i], ratios.escape.max[[i]], snp_stat_df), mc.cores = 20)
Sys.time()
...
for(i in 1:length(sample_index)){
  ...
  data = filt_macaca_vcfs_noBadbins[[sample_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
escape_mean = rep(NA, length = length(annot_macaca_vcfs))
escape_var = rep(NA, length = length(annot_macaca_vcfs))
num_escape_snps = rep(NA, length = length(annot_macaca_vcfs))
inact_mean = rep(NA, length = length(annot_macaca_vcfs))
inact_var = rep(NA, length = length(annot_macaca_vcfs))
num_inact_snps = rep(NA, length = length(annot_macaca_vcfs))

for(i in 1:length(annot_macaca_vcfs)){

  data = annot_macaca_vcfs[[i]]
  if(is.null(dim(data))){next}
  escape_index = (plyr::round_any(data$Pos, 1000000, ceiling)/1e6) %in% bad_macaca_bins #Get the SNPs located in the escape bins
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(chrX_skewed_index)){
  ...
  data = filt_macaca_vcfs_noBadbins[[chrX_skewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(chrX_Nonskewed_index)){
  ...
  data = filt_macaca_vcfs_noBadbins[[chrX_Nonskewed_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
...
for(i in 1:length(enoughSNPs_index)){
  ...
  data = filt_macaca_vcfs_noBadbins[[enoughSNPs_index[i]]]
  data = data[data$Pos %in% snp_stat_df$snp_pos[snp_stat_df$filter], ]
  ref_ratios = data$ref_counts / (data$ref_counts + data$alt_counts)
  all_ref_ratios = c(all_ref_ratios, ref_ratios)
...
ggplot(skew_bin_snp_df, aes(x = ref_ratios, y = bin_label)) + geom_density_ridges(rel_min_height = 0.01, scale = 0.95, fill = species_colors[1] ) + xlim(0,1) +
...
#ggsave(filename = 'macaca_chrX_skew_binned_refRatio_ridgePlot.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/species_skew_binned_refRatios/', 
#       device = 'pdf', useDingbats = F, height = 6, width = 6)

macaca_skew_bin_snp_df = skew_bin_snp_df
save(macaca_skew_bin_snp_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/macaca_skew_bin_snp_df.Rdata')
```



```{r}
macaca_skew_and_stats_df = data.frame(sample_id = wig_samples, 
...
macaca_skew_and_stats_df
```


```{r}

save(macaca_skew_and_stats_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/macaca_mulatta/macaca_skew_and_stats_binFilt_df_4_27_23.Rdata')
