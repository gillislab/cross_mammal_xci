...
writeLines(mouse_skew_and_stats_df$sample_id[mouse_skew_and_stats_df$num_good_snps_no_escape >= 10],
           '/home/werner/projects/cross_species_XCI/code/female_metadata/verified_X_samples/mus_musculus_verified_X_SRR.txt')
```
...
all_mouse_samps = length(readLines('/home/werner/projects/cross_species_XCI/code/female_metadata/mus_musculus_female_SRR.txt'))
...
kept_mouse_samps = nrow(mouse_skew_and_stats_df[mouse_skew_and_stats_df$num_good_snps_no_escape >= 10, ])
...
num_samps = c(all_cow_samps, all_sheep_samps, all_goat_samps, all_horse_samps, all_dog_samps, all_pig_samps, 
              all_rat_samps,all_mouse_samps, all_macaca_samps, all_human_samps,
              kept_cow_samps, kept_sheep_samps, kept_goat_samps, kept_horse_samps, kept_dog_samps, kept_pig_samps, 
              kept_rat_samps, kept_mouse_samps, kept_macaca_samps, kept_human_samps)

species_label = c('Cow','Sheep','Goat','Horse','Dog','Pig','Rat', 'Mouse','Macaca','Human','Cow','Sheep','Goat','Horse','Dog','Pig','Rat','Mouse','Macaca','Human')
...
#ggsave(filename ='sample_num_barplot_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/schematic_plots/', 
#       device = 'pdf', useDingbats = F, width = 7, height = 4)


save(species_sample_df, file = '/home/werner/projects/cross_species_XCI/final_plots/R/data_for_plots/species_sample_df.Rdata')
...
cow_snp_nums = filter(cow_skew_and_stats_df, num_good_snps_no_escape >= 10)$num_good_snps_no_escape

pig_snp_nums = filter(pig_skew_and_stats_df,num_good_snps_no_escape >= 10)$num_good_snps_no_escape

macaca_snp_nums = filter(macaca_skew_and_stats_df,  num_good_snps_no_escape >= 10)$num_good_snps_no_escape

rat_snp_nums = filter(rat_skew_and_stats_df, num_good_snps_no_escape >= 10)$num_good_snps_no_escape

mouse_snp_nums = filter(mouse_skew_and_stats_df,  num_good_snps_no_escape >= 10)$num_good_snps_no_escape

dog_snp_nums = filter(dog_skew_and_stats_df, num_good_snps_no_escape >= 10)$num_good_snps_no_escape

goat_snp_nums = filter(goat_skew_and_stats_df, num_good_snps_no_escape >= 10)$num_good_snps_no_escape

sheep_snp_nums = filter(sheep_skew_and_stats_df, num_good_snps_no_escape >= 10)$num_good_snps_no_escape

horse_snp_nums = filter(horse_skew_and_stats_df,  num_good_snps_no_escape >= 10)$num_good_snps_no_escape

human_snp_nums = filter(human_skew_and_stats_df,  num_snps >= 10)$num_snps


all_snps = c(cow_snp_nums, pig_snp_nums, macaca_snp_nums, rat_snp_nums, mouse_snp_nums, dog_snp_nums, goat_snp_nums, sheep_snp_nums, horse_snp_nums, human_snp_nums)
species_label = c(rep('Cow', length = length(cow_snp_nums)), rep('Pig', length = length(pig_snp_nums)), rep('Macaca', length = length(macaca_snp_nums)),
                  rep('Rat', length = length(rat_snp_nums)), rep('Mouse', length = length(mouse_snp_nums)), rep('Dog', length = length(dog_snp_nums)), 
                  rep('Goat', length = length(goat_snp_nums)),
...
ggsave(filename = 'number_of_snps_persample_violin_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/ref_snp_filtering/', device = 'pdf', useDingbats = F, 
       height = 4, width = 6)
```
...
summary_num_snps_df = num_snps_df %>% group_by(species) %>% summarize(num_samps = length(snp_counts), mean_snps = mean(snp_counts), sd_snps = sd(snp_counts))
...
dim(mouse_skew_and_stats_df[mouse_skew_and_stats_df$num_good_snps_no_escape >= 10 & mouse_skew_and_stats_df$autosome_imbalance == F, ])
...
sample_index = mouse_skew_and_stats_df$num_good_snps_no_escape >=10 & mouse_skew_and_stats_df$autosome_imbalance == F
hist(unfold(mouse_skew_and_stats_df$est_skew_no_escape[sample_index]), breaks = seq(0,1,.005), 
     main = sprintf('Mouse population XCI ratios, # samples: %i', sum(sample_index)), xlab = 'Estimated sample XCI ratio')
```
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/mus_musculus/max_snp_vcf_binFilt_4_22_24.Rdata')
all_pos = c()
all_variants = c()
...
all_mouse_variants = data.frame(position = all_pos, variants = all_variants)
mouse_inbred_vec = table(paste0(all_mouse_variants$position, all_mouse_variants$variants))
mouse_inbred_df = data.frame(variant = names(mouse_inbred_vec), num_samps_detected = as.numeric(mouse_inbred_vec), fraction_total_samps = as.numeric(mouse_inbred_vec) / length(annot_mouse_vcfs))
...
species_frac_snps_detected_once = c(
  ...
  sum(mouse_inbred_df$num_samps_detected == 1) / nrow(mouse_inbred_df),
...
species_total_num_snps = c(nrow(dog_inbred_df), nrow(horse_inbred_df), nrow(pig_inbred_df), nrow(cow_inbred_df), nrow(goat_inbred_df), nrow(sheep_inbred_df),
                           nrow(rat_inbred_df), nrow(mouse_inbred_df), nrow(macaca_inbred_df), nrow(human_inbred_df))

species_label = c('Dog','Horse','Pig','Cow','Goat','Sheep','Rat','Mouse','Macaca','Human')
...
ggplot(snps_detected_once_df, aes(x = total_snps, label = label, y = ratio_detected_once, color = label)) + geom_point(size = 3) + geom_text(vjust = -1) +
  scale_color_manual(values = species_palette) + xlab('Total # of SNPs detected') + ylab('Fraction of SNPs detected once') + ylim(.25, .7)
```
...
compare_skew_and_singletons('Mouse', mouse_inbred_df, annot_mouse_vcfs, mouse_skew_and_stats_df)
...
compare_heterozygosity_and_xci = function(inbred_df, annot_vcf, species_name, skew_df, sample_size_filter = 10){
  ...
  print(p1)
  file_name = sprintf('%s_het_exp_skew_scatter_v2.pdf', species_name)
  ...
  print(p2)
  file_name = sprintf('%s_het_exp_var_scatter_v2.pdf', species_name)
...
mouse_het_df = compare_heterozygosity_and_xci(mouse_inbred_df, annot_mouse_vcfs, 'mouse', mouse_skew_and_stats_df)
...
ggsave(filename = 'human_het_exp_skew_scatter_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/heterozygousity/', device = 'pdf', 
       useDingbats = F, height = 3, width = 4)
...
ggsave(filename = 'human_het_exp_var_scatter_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/heterozygousity/', device = 'pdf', 
       useDingbats = F, height = 3, width = 4)
...
ggsave(filename = 'species_het_frac_distr_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/heterozygousity/', device = 'pdf', 
...
mouse_skew_corr = cor(mouse_het_df$het_fraction, mouse_het_df$est_skew_no_escape, method = 'spearman', use = 'complete.obs')
mouse_skew_corr_pval = cor_pval(x = mouse_het_df$het_fraction, y = mouse_het_df$est_skew_no_escape, method_cor = 'spearman', num_permutes = n_per)
mouse_var_corr = cor(mouse_het_df$het_fraction, mouse_het_df$est_var_no_escape, method = 'spearman', use = 'complete.obs')
mouse_var_corr_pval = cor_pval(x = mouse_het_df$het_fraction, y = mouse_het_df$est_var_no_escape, method_cor = 'spearman', num_permutes = n_per)
...
#All correlations
corr_skew_vec = c(cow_skew_corr, rat_skew_corr, mouse_skew_corr, macaca_skew_corr, dog_skew_corr, pig_skew_corr, 
                  sheep_skew_corr, goat_skew_corr, horse_skew_corr, human_skew_corr )
corr_var_vec = c(cow_var_corr, rat_var_corr, mouse_var_corr, macaca_var_corr, dog_var_corr, pig_var_corr, 
                 sheep_var_corr, goat_var_corr, horse_var_corr, human_var_corr )
corrs_vec = c(corr_skew_vec, corr_var_vec)
#all correlation pvalues
corr_skew_pval_vec = c(cow_skew_corr_pval, rat_skew_corr_pval, mouse_skew_corr_pval, macaca_skew_corr_pval, dog_skew_corr_pval, pig_skew_corr_pval, 
                       sheep_skew_corr_pval, goat_skew_corr_pval, horse_skew_corr_pval, human_skew_corr_pval )
corr_var_pval_vec = c(cow_var_corr_pval, rat_var_corr_pval, mouse_var_corr_pval, macaca_var_corr_pval, dog_var_corr_pval, pig_var_corr_pval, 
...
species_vec = c('Cow','Rat', 'Mouse','Macaca','Dog','Pig','Sheep','Goat','Horse','Human')
...
het_corr_df$corrs_label = factor(het_corr_df$corrs_label, levels = c('XCI ratio','Allelic expression variance'))
...
ggsave(filename = 'het_corr_violin_plot_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/heterozygousity/', device = 'pdf', 
       useDingbats = F, height = 4, width = 6)
```
```{r}

save(het_corr_df, file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/heterozygousity/het_corr_df_v2.Rdata')
...
load( file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/heterozygousity/het_corr_df.Rdata')
...
mouse_folded_skews = filter(mouse_skew_and_stats_df, num_good_snps_no_escape >= 10 & autosome_imbalance == F)$est_skew_no_escape
mouse_squared_error = get_cell_num_error_ests(mouse_folded_skews, n_min, n_max)
sprintf('mouse estimated cell num: %i', n_vec[which.min(mouse_squared_error)])
...
species_squared_error_df = data.frame(n_vec = n_vec, horse_error = horse_squared_error, dog_error = dog_squared_error, cow_error = cow_squared_error, 
                                      sheep_error = sheep_squared_error, goat_error = goat_squared_error, rat_error = rat_squared_error, 
                                      mouse_error = mouse_squared_error, macaca_error = macaca_squared_error, pig_error = pig_squared_error)
...
pdf(file = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/error_plots/mouse_x_cell_num_error.pdf', useDingbats = F, height = 4, width = 6)
...
mouse_ci = get_bootstrap_ci(mouse_folded_skews, cellN_min = 2, cellN_max = 200, num_bootstraps = 2000, ci = .95)
...
all_skew_and_stats = list(horse_skew_and_stats_df, dog_skew_and_stats_df, cow_skew_and_stats_df, sheep_skew_and_stats_df, 
                          goat_skew_and_stats_df, rat_skew_and_stats_df, mouse_skew_and_stats_df, macaca_skew_and_stats_df, pig_skew_and_stats_df)
 
names(all_skew_and_stats) = c('Horse','Dog','Cow','Sheep','Goat','Rat','Mouse','Macaca','Pig')
all_skew_and_stats
```
...
for(j in 1:length(all_skew_and_stats)){
  ...
  ggsave(plot = g, filename = sprintf('%s_normal_fits_binnedFilt_auto_balanced_v2.pdf', species_name) ,
         path ='/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
         width = 6, height = 4, useDingbats = FALSE)
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/homo_sapiens/agg_gtex_skew_and_stats_df.Rdata')
...
ggsave(plot = g, filename = 'agg_donor_human_normal_fits_v2.pdf' ,
 path ='/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
 width = 6, height = 4, useDingbats = FALSE)
...
all_ci = list(human_ci, horse_ci, cow_ci, pig_ci, goat_ci, sheep_ci, macaca_ci, rat_ci, mouse_ci, dog_ci)
all_ci_df = data.frame(do.call(rbind, all_ci))
all_ci_df$species = c('Human','Horse','Cow','Pig','Goat','Sheep','Macaca','Rat','Mouse','Dog')
...
ggsave(filename = 'log2_chrX_cellnumests_with_CI_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
...
ggsave(filename = 'chrX_cellnumests_with_CI_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 5)
...
ggsave(filename = 'evo_log2_chrX_cellnumests_with_CI_v2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 5)


ggplot(all_ci_df, aes(y = species, x = cell_num_est, color = species)) + geom_point(size = 3) + geom_errorbar(aes(xmin = lower_ci, xmax = upper_ci)) +
  scale_color_manual(values = species_palette) + xlab('Cell # estimate') + ggtitle('Chromosome X') + ylab('') +
  scale_y_discrete(limits=rev) + scale_x_continuous(breaks=c(2,4, 8, 16, 32, 64, 128))
ggsave(filename = 'evo_chrX_cellnumests_with_CI_log2_xaxisv2.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 5)
...
Compare the number of SNPs and samples per species to the cell number estimates

```{r}

index = match(summary_num_snps_df$species, all_ci_df$species)
all_ci_df = all_ci_df[index, ]

cell_num_stats_df = data.frame(species = all_ci_df$species, estimated_cell_num = all_ci_df$cell_num_est, 
                               sample_number = summary_num_snps_df$num_samps, mean_snp = summary_num_snps_df$mean_snps )
...
ggsave(filename = 'cell_num_ests_and_sample_num_scatter_log2X.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 5)

ggplot(cell_num_stats_df, aes(y = log10(sample_number), x = estimated_cell_num, color = species)) + geom_point(size = 3) +
  scale_color_manual(values = species_palette) + ylab('log10( Total sample number )') + xlab('Cell # estimate') + ggtitle(sprintf('Correlation: %1.3f', ori_corr))
ggsave(filename = 'cell_num_ests_and_sample_num_scatter.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 5)
...
ggplot(cell_num_stats_df, aes(y =  mean_snp, x = estimated_cell_num, color = species)) + geom_point(size = 3) +
  ...
  scale_x_continuous(breaks=c(2,4, 8, 16, 32, 64, 128))
ggsave(filename = 'cell_num_ests_and_mean_snp_scatter_log2X.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 5)

ggplot(cell_num_stats_df, aes(y =  mean_snp, x = estimated_cell_num, color = species)) + geom_point(size = 3) +
  scale_color_manual(values = species_palette)  + ylab('Mean SNP # per sample') + xlab('Cell # estimate') + ggtitle(sprintf('Correlation: %1.3f', ori_corr))
ggsave(filename = 'cell_num_ests_and_mean_snp_scatter.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 5)
```

################################################

Now downsample all species to 130 to match the macaca sample size and get downsampled error bars about the cell num estimate
...
get_downsamp_cellNum_est = function(folded_skews, cellN_min, cellN_max, num_samples, ci, num_downsamp){

  n_vec = cellN_min:cellN_max
  
  downsamp_cellNum_ests = vector(mode = 'numeric', length = num_samples)
  for(i in 1:num_samples){
    downsamp_folded_skews = sample(folded_skews, size = num_downsamp, replace = T)
    squared_error = get_cell_num_error_ests(downsamp_folded_skews, n_min, n_max)
    downsamp_cellNum_ests[i] = n_vec[which.min(squared_error)]
  }
  #Order and return the upper and lower confidence intervals
  downsamp_cellNum_ests = downsamp_cellNum_ests[order(downsamp_cellNum_ests)]
  lower_ci = downsamp_cellNum_ests[((1-ci)*num_samples)/2 + 1]
  upper_ci = downsamp_cellNum_ests[num_samples - ((1-ci)*num_samples)/2]

  return(c('lower_ci' = lower_ci, 'cell_num_est' = mean(downsamp_cellNum_ests), 'upper_ci' = upper_ci))
}


horse_downsamp_ci = get_downsamp_cellNum_est(horse_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
cow_downsamp_ci = get_downsamp_cellNum_est(cow_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
dog_downsamp_ci = get_downsamp_cellNum_est(dog_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
sheep_downsamp_ci = get_downsamp_cellNum_est(sheep_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
goat_downsamp_ci = get_downsamp_cellNum_est(goat_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
pig_downsamp_ci = get_downsamp_cellNum_est(pig_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
rat_downsamp_ci = get_downsamp_cellNum_est(rat_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
mouse_downsamp_ci = get_downsamp_cellNum_est(mouse_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)

human_folded_skews = agg_gtex_skew_and_stats_df$est_skew[agg_gtex_skew_and_stats_df$num_good_snps >= 10]
human_downsamp_ci = get_downsamp_cellNum_est(human_folded_skews, cellN_min = 2, cellN_max = 200, num_samples = 2000, ci = .95, num_downsamp = 130)
...
all_downsamp_ci = list(human_downsamp_ci, horse_downsamp_ci, cow_downsamp_ci, pig_downsamp_ci, goat_downsamp_ci, 
                       sheep_downsamp_ci, rat_downsamp_ci, mouse_downsamp_ci, dog_downsamp_ci)
all_downsamp_ci_df = data.frame(do.call(rbind, all_downsamp_ci))
all_downsamp_ci_df$species = c('Human downsampled','Horse downsampled','Cow downsampled','Pig downsampled','Goat downsampled',
                               'Sheep downsampled','Rat downsampled','Mouse downsampled','Dog downsampled')
all_downsamp_ci_df = all_downsamp_ci_df %>% arrange(cell_num_est)
#all_downsamp_ci_df$species = factor(all_downsamp_ci_df$species, levels = c(all_downsamp_ci_df$species))
all_downsamp_ci_df$label = rep('Downsampled estimate', length = nrow(all_downsamp_ci_df))
all_downsamp_ci_df
...
all_ci_df$label = rep('Original estimate', length = nrow(all_ci_df))
compare_ci_df = rbind(all_ci_df, all_downsamp_ci_df)
compare_ci_df = compare_ci_df %>% filter(species != 'Macaca')

compare_ci_df$species = factor(compare_ci_df$species, levels = c('Dog', 'Dog downsampled','Horse', 'Horse downsampled','Cow', 'Cow downsampled','Goat', 'Goat downsampled',
                                                                 'Sheep', 'Sheep downsampled','Pig', 'Pig downsampled','Rat', 'Rat downsampled','Mouse', 'Mouse downsampled',
                                                                 'Human', 'Human downsampled'))
compare_ci_df$color_label = sapply( strsplit(as.character(compare_ci_df$species), split = ' ', fixed = T),'[[', 1)
...
ggplot(compare_ci_df, aes(y = species, x = log2(cell_num_est), color = color_label, alpha = label)) + geom_point(size = 3) + 
  ...
  scale_color_manual(values = species_palette) + xlim(2, 7) + xlab('Cell divisions (log2 cell # estimate)') + ggtitle('Chromosome X') + ylab('') +
  scale_y_discrete(limits=rev)
ggsave(filename = 'downsamp_cell_num_ests_to_macaca_log2X.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 7)


ggplot(compare_ci_df, aes(y = species, x = cell_num_est, color = color_label, alpha = label)) + geom_point(size = 3) + 
  geom_errorbar(aes(xmin = lower_ci, xmax = upper_ci)) +
  scale_alpha_manual(values = c('Downsampled estimate' = .5, 'Original estimate' = 1)) +
  scale_color_manual(values = species_palette) + xlim(2, 7) + xlab('Cell # estimate') + ggtitle('Chromosome X') + ylab('') +
  scale_y_discrete(limits=rev) + scale_x_continuous(breaks=c(2,4, 8, 16, 32, 64, 128))
ggsave(filename = 'downsamp_cell_num_ests_to_macaca.pdf', path = '/home/werner/projects/cross_species_XCI/code/graphs/est_cell_num/', 
       device = 'pdf', useDingbats = F, height = 4, width = 7)
