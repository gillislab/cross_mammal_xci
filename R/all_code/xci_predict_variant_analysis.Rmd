...
temp_index = human_results$auc >= .75 & human_results$reference_biased_index == F
table(temp_index)
human_results$variants[temp_index]
...
```{r}
all_sheep_downsample_df = data.frame(variants = c(), ref_index = c(),
                                 n_pos = c(), n_neg = c(),
                                 auc = c(), auc_pval = c(),
                                 auc_fdr_pval = c(),
                                 gene = c(), down_sample = c())
for(s in 1:10){
  ...
  #Get a sample of 300
  down_sample_index = sample(sample_index, size = 300)
  sheep_results_down_1 = xci_skew_predict_variant_experiment(sheep_skew_and_stats_df, annot_sheep_vcfs, down_sample_index, sheep_snp_stat_df)
  Sys.time() - start
  
  #Get the gene annotations for the variants
  sheep_variants = sheep_results_down_1$variants
  sheep_snp_pos = as.numeric(sapply(1:length(sheep_variants), 
                                         function(i) substr(sheep_variants[i],1,nchar(sheep_variants[i])-2)))
  sheep_x_meta = sheep_x_gtf[sheep_x_gtf$gbkey == 'Gene'& !is.na(sheep_x_gtf$gbkey),]
  x_gene_starts = sheep_x_meta$start
  x_gene_ends = sheep_x_meta$end
  #Add gene annotations
  ...
  #Put it all in a dataframe
  sheep_temp_results_df = data.frame(variants = sheep_results_down_1$variants, ref_index = sheep_results_down_1$reference_biased_index,
                                   n_pos = sheep_results_down_1$n_pos, n_neg = sheep_results_down_1$n_neg,
                                   auc = sheep_results_down_1$auc, auc_pval = sheep_results_down_1$auc_pval,
                                   auc_fdr_pval = p.adjust(sheep_results_down_1$auc_pval, method = 'BH'),
                                   gene = sheep_gene_annots, down_sample = rep(sprintf('Sample %s', s), 
                                                                             length = length(sheep_results_down_1$variants)))
  
  #Add to the complete dataframe
  all_sheep_downsample_df = rbind(all_sheep_downsample_df,sheep_temp_results_df  )

}

all_sheep_downsample_df

gene_hits_down_sheep = all_sheep_downsample_df %>% filter(auc >= .75 & auc_fdr_pval <= .05 & ref_index == F) %>% group_by(down_sample, gene) %>% summarize(n = length(gene)) %>% group_by(gene) %>% summarize(n = length(gene)) %>% arrange(desc(n))

gene_hits_down_sheep$gene = factor(gene_hits_down_sheep$gene, levels = c(gene_hits_down_sheep$gene))
ggplot(gene_hits_down_sheep, aes(x = gene, y = n)) + geom_col() + theme(axis.text.x = element_text(angle = 90)) +
  ylab('Number of downsamples') + xlab('Genes with skew associated variants') + ggtitle('Sheep')
```

```{r}

save(all_sheep_downsample_df, 
     file = '/home/werner/projects/cross_species_XCI/data/species_data_analysis/snp_skew_auc/all_sheep_downsample_df.Rdata')
...
load('/home/werner/projects/cross_species_XCI/data/species_data_analysis/snp_skew_auc/all_sheep_downsample_df.Rdata')
...
ori_variant_data = filter(snp_skew_auc_with_meta_df, label == 'Sheep' & gene == 'CXCR3')
downSamp_variant_data = filter(all_sheep_downsample_df, gene == 'CXCR3') %>% group_by(variants) %>% summarize(mean_auc = mean(auc), max_auc = max(auc))
...
ori_variant_data = filter(snp_skew_auc_with_meta_df, label == 'Sheep' & gene == 'LOC114111559')
downSamp_variant_data = filter(all_sheep_downsample_df, gene == 'LOC114111559') %>% group_by(variants) %>% summarize(mean_auc = mean(auc), max_auc = max(auc))
...
comb_test_df = Reduce(rbind, list(arhgap6_var_1_df, arhgap6_var_2_df, arhgap6_var_3_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('Horse ARHGAP6') + ylab('XCI skews') + ggtitle('Horse ARHGAP6')
...
comb_test_df = Reduce(rbind, list(gpc4_var_1_df,gpc4_var_2_df, gpc4_var_3_df, gpc4_var_4_df, gpc4_var_5_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('Horse GPC4') + ylab('XCI skews') + ggtitle('Horse GPC4')
...
comb_test_df = Reduce(rbind, list(plcxd1_var_1_df,plcxd1_var_2_df,plcxd1_var_3_df,plcxd1_var_4_df,plcxd1_var_5_df,plcxd1_var_6_df,plcxd1_var_7_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('Horse PLCXD1') + ylab('XCI skews') + ggtitle('Horse PLCXD1')
...
comb_test_df = Reduce(rbind, list(arse_var_1_df, arse_var_2_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('Horse ARSE') + ylab('XCI skews') + ggtitle('Horse ARSE')
...
comb_test_df = Reduce(rbind, list(msl3_var_1_df, msl3_var_2_df, msl3_var_3_df, msl3_var_4_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('goat MSL3') + ylab('XCI skews') + ggtitle('goat MSL3')
...
comb_test_df = Reduce(rbind, list(MXRA5_var_1_df, MXRA5_var_2_df, MXRA5_var_3_df, MXRA5_var_4_df, MXRA5_var_5_df, MXRA5_var_6_df, MXRA5_var_7_df, MXRA5_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('goat MXRA5') + ylab('XCI skews') + ggtitle('goat MXRA5')
...
comb_test_df = Reduce(rbind, list(SHROOM2_var_1_df, SHROOM2_var_2_df, SHROOM2_var_3_df, SHROOM2_var_4_df, SHROOM2_var_5_df, SHROOM2_var_6_df, SHROOM2_var_7_df, SHROOM2_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('goat SHROOM2') + ylab('XCI skews') + ggtitle('goat SHROOM2')
...
comb_test_df = Reduce(rbind, list(loc102_var_1_df, loc102_var_2_df, loc102_var_3_df, loc102_var_4_df, loc102_var_5_df, loc102_var_6_df, loc102_var_7_df, loc102_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('goat LOC102168615') + ylab('XCI skews') + ggtitle('goat LOC102168615')
...
comb_test_df = Reduce(rbind, list(MAMLD1_var_1_df, MAMLD1_var_2_df, MAMLD1_var_3_df, MAMLD1_var_4_df, MAMLD1_var_5_df, MAMLD1_var_6_df, MAMLD1_var_7_df, MAMLD1_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('cow MAMLD1') + ylab('XCI skews') + ggtitle('cow MAMLD1')
...
comb_test_df = Reduce(rbind, list(MID2_var_1_df, MID2_var_2_df, MID2_var_3_df, MID2_var_4_df, MID2_var_5_df, MID2_var_6_df, MID2_var_7_df, MID2_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('cow MID2') + ylab('XCI skews') + ggtitle('cow MID2')
...
comb_test_df = Reduce(rbind, list(TMEM164_var_1_df, TMEM164_var_2_df, TMEM164_var_3_df, TMEM164_var_4_df, TMEM164_var_5_df, TMEM164_var_6_df, TMEM164_var_7_df, TMEM164_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('cow TMEM164') + ylab('XCI skews') + ggtitle('cow TMEM164')
...
comb_test_df = Reduce(rbind, list(KDM6A_var_1_df, KDM6A_var_2_df, KDM6A_var_3_df, KDM6A_var_4_df, KDM6A_var_5_df, KDM6A_var_6_df, KDM6A_var_7_df, KDM6A_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('cow KDM6A') + ylab('XCI skews') + ggtitle('cow KDM6A')
...
Checking out the Snps associated with XCI skews in pig
```{r}

snp_skew_auc_with_meta_df %>% filter(label == 'Pig', skew_auc >= .75 )

```

```{r}
filter(pig_snp_stat_df, gene == 'PDZD11' & num_samples_present >= 10) %>% arrange(desc(num_samples_present))
...
```{r}
sample_index = which(pig_skew_and_stats_df$num_good_snps_no_escape >= 10 & pig_skew_and_stats_df$autosome_imbalance == F)
filt_annot_pig_vcfs = annot_pig_vcfs[sample_index]
pig_XCI_skews = pig_skew_and_stats_df$est_skew_no_escape[sample_index]


#EIF1AX
EIF1AX_var_1_df =  get_var_data(filt_annot_pig_vcfs,'16432488', pig_XCI_skews)
EIF1AX_var_2_df =  get_var_data(filt_annot_pig_vcfs,'16431676', pig_XCI_skews)
EIF1AX_var_3_df =  get_var_data(filt_annot_pig_vcfs,'16430163', pig_XCI_skews)
EIF1AX_var_4_df =  get_var_data(filt_annot_pig_vcfs,'16430537', pig_XCI_skews)
EIF1AX_var_5_df =  get_var_data(filt_annot_pig_vcfs,'16430872', pig_XCI_skews)
EIF1AX_var_6_df =  get_var_data(filt_annot_pig_vcfs,'16431193', pig_XCI_skews)
EIF1AX_var_7_df =  get_var_data(filt_annot_pig_vcfs,'16429689', pig_XCI_skews)
EIF1AX_var_8_df =  get_var_data(filt_annot_pig_vcfs,'16432829', pig_XCI_skews)

comb_test_df = Reduce(rbind, list(EIF1AX_var_1_df, EIF1AX_var_2_df, EIF1AX_var_3_df, EIF1AX_var_4_df, EIF1AX_var_5_df, EIF1AX_var_6_df, EIF1AX_var_7_df, EIF1AX_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('pig EIF1AX') + ylab('XCI skews') + ggtitle('pig EIF1AX')


#PDZD11
PDZD11_var_1_df =  get_var_data(filt_annot_pig_vcfs,'56316733', pig_XCI_skews)
PDZD11_var_2_df =  get_var_data(filt_annot_pig_vcfs,'56316986', pig_XCI_skews)
PDZD11_var_3_df =  get_var_data(filt_annot_pig_vcfs,'56316700', pig_XCI_skews)
PDZD11_var_4_df =  get_var_data(filt_annot_pig_vcfs,'56318020', pig_XCI_skews)

comb_test_df = Reduce(rbind, list(PDZD11_var_1_df, PDZD11_var_2_df, PDZD11_var_3_df, PDZD11_var_4_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('pig PDZD11') + ylab('XCI skews') + ggtitle('pig PDZD11')
...
comb_test_df = Reduce(rbind, list(MXRA5_var_1_df, MXRA5_var_2_df, MXRA5_var_3_df, MXRA5_var_4_df, MXRA5_var_5_df, MXRA5_var_6_df, MXRA5_var_7_df, MXRA5_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('sheep MXRA5') + ylab('XCI skews') + ggtitle('sheep MXRA5')
...
Checking out the Snps associated with XCI skews in dog
```{r}

snp_skew_auc_with_meta_df %>% filter(label == 'Dog', skew_auc >= .75 )

```

```{r}
filter(dog_snp_stat_df, gene == 'SHROOM2' & num_samples_present >= 10) %>% arrange(desc(num_samples_present))

```

```{r}
sample_index = which(dog_skew_and_stats_df$num_good_snps_no_escape >= 10 & dog_skew_and_stats_df$autosome_imbalance == F)
filt_annot_dog_vcfs = annot_dog_vcfs[sample_index]
dog_XCI_skews = dog_skew_and_stats_df$est_skew_no_escape[sample_index]


#SHROOM2
SHROOM2_var_1_df =  get_var_data(filt_annot_dog_vcfs,'6597148', dog_XCI_skews)
SHROOM2_var_2_df =  get_var_data(filt_annot_dog_vcfs,'6544128', dog_XCI_skews)
SHROOM2_var_3_df =  get_var_data(filt_annot_dog_vcfs,'6543788', dog_XCI_skews)
SHROOM2_var_4_df =  get_var_data(filt_annot_dog_vcfs,'6545490', dog_XCI_skews)
SHROOM2_var_5_df =  get_var_data(filt_annot_dog_vcfs,'6539771', dog_XCI_skews)
SHROOM2_var_6_df =  get_var_data(filt_annot_dog_vcfs,'6597798', dog_XCI_skews)
SHROOM2_var_7_df =  get_var_data(filt_annot_dog_vcfs,'6544974', dog_XCI_skews)
SHROOM2_var_8_df =  get_var_data(filt_annot_dog_vcfs,'6598640', dog_XCI_skews)

comb_test_df = Reduce(rbind, list(SHROOM2_var_1_df, SHROOM2_var_2_df, SHROOM2_var_3_df, SHROOM2_var_4_df, SHROOM2_var_5_df, SHROOM2_var_6_df, SHROOM2_var_7_df, SHROOM2_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('dog SHROOM2') + ylab('XCI skews') + ggtitle('dog SHROOM2')
...
Checking out the Snps associated with XCI skews in macaca
```{r}

snp_skew_auc_with_meta_df %>% filter(label == 'Macaca', skew_auc >= .75 )

```

```{r}
filter(macaca_snp_stat_df, gene == 'IKBKG' & num_samples_present >= 10) %>% arrange(desc(num_samples_present))

```

```{r}
sample_index = which(macaca_skew_and_stats_df$num_good_snps_no_escape >= 10 & macaca_skew_and_stats_df$autosome_imbalance == F)
filt_annot_macaca_vcfs = annot_macaca_vcfs[sample_index]
macaca_XCI_skews = macaca_skew_and_stats_df$est_skew_no_escape[sample_index]


#P2RY8
P2RY8_var_1_df =  get_var_data(filt_annot_macaca_vcfs,'1251490', macaca_XCI_skews)
P2RY8_var_2_df =  get_var_data(filt_annot_macaca_vcfs,'1323652', macaca_XCI_skews)
P2RY8_var_3_df =  get_var_data(filt_annot_macaca_vcfs,'1252576', macaca_XCI_skews)
P2RY8_var_4_df =  get_var_data(filt_annot_macaca_vcfs,'1252215', macaca_XCI_skews)
P2RY8_var_5_df =  get_var_data(filt_annot_macaca_vcfs,'1252095', macaca_XCI_skews)
P2RY8_var_6_df =  get_var_data(filt_annot_macaca_vcfs,'1252835', macaca_XCI_skews)
P2RY8_var_7_df =  get_var_data(filt_annot_macaca_vcfs,'1252679', macaca_XCI_skews)
P2RY8_var_8_df =  get_var_data(filt_annot_macaca_vcfs,'1252237', macaca_XCI_skews)

comb_test_df = Reduce(rbind, list(P2RY8_var_1_df, P2RY8_var_2_df, P2RY8_var_3_df, P2RY8_var_4_df, P2RY8_var_5_df, P2RY8_var_6_df, P2RY8_var_7_df, P2RY8_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('macaca P2RY8') + ylab('XCI skews') + ggtitle('macaca P2RY8')


#AKAP17A
AKAP17A_var_1_df =  get_var_data(filt_annot_macaca_vcfs,'1381506', macaca_XCI_skews)
AKAP17A_var_2_df =  get_var_data(filt_annot_macaca_vcfs,'1381726', macaca_XCI_skews)
AKAP17A_var_3_df =  get_var_data(filt_annot_macaca_vcfs,'1381438', macaca_XCI_skews)
AKAP17A_var_4_df =  get_var_data(filt_annot_macaca_vcfs,'1381338', macaca_XCI_skews)
AKAP17A_var_5_df =  get_var_data(filt_annot_macaca_vcfs,'1381629', macaca_XCI_skews)
AKAP17A_var_6_df =  get_var_data(filt_annot_macaca_vcfs,'1380290', macaca_XCI_skews)
AKAP17A_var_7_df =  get_var_data(filt_annot_macaca_vcfs,'1381625', macaca_XCI_skews)
AKAP17A_var_8_df =  get_var_data(filt_annot_macaca_vcfs,'1381679', macaca_XCI_skews)

comb_test_df = Reduce(rbind, list(AKAP17A_var_1_df, AKAP17A_var_2_df, AKAP17A_var_3_df, AKAP17A_var_4_df, AKAP17A_var_5_df, AKAP17A_var_6_df, AKAP17A_var_7_df, AKAP17A_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('macaca AKAP17A') + ylab('XCI skews') + ggtitle('macaca AKAP17A')


#IDS
IDS_var_1_df =  get_var_data(filt_annot_macaca_vcfs,'145507086', macaca_XCI_skews)
IDS_var_2_df =  get_var_data(filt_annot_macaca_vcfs,'145508674', macaca_XCI_skews)
IDS_var_3_df =  get_var_data(filt_annot_macaca_vcfs,'145510929', macaca_XCI_skews)
IDS_var_4_df =  get_var_data(filt_annot_macaca_vcfs,'145518493', macaca_XCI_skews)

comb_test_df = Reduce(rbind, list(IDS_var_1_df, IDS_var_2_df, IDS_var_3_df, IDS_var_4_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('macaca IDS') + ylab('XCI skews') + ggtitle('macaca IDS')


#IKBKG
IKBKG_var_1_df =  get_var_data(filt_annot_macaca_vcfs,'150651623', macaca_XCI_skews)
IKBKG_var_2_df =  get_var_data(filt_annot_macaca_vcfs,'150664599', macaca_XCI_skews)
IKBKG_var_3_df =  get_var_data(filt_annot_macaca_vcfs,'150652464', macaca_XCI_skews)
IKBKG_var_4_df =  get_var_data(filt_annot_macaca_vcfs,'150662826', macaca_XCI_skews)
IKBKG_var_5_df =  get_var_data(filt_annot_macaca_vcfs,'150661323', macaca_XCI_skews)
IKBKG_var_6_df =  get_var_data(filt_annot_macaca_vcfs,'150656126', macaca_XCI_skews)
IKBKG_var_7_df =  get_var_data(filt_annot_macaca_vcfs,'150656118', macaca_XCI_skews)
IKBKG_var_8_df =  get_var_data(filt_annot_macaca_vcfs,'150656114', macaca_XCI_skews)

comb_test_df = Reduce(rbind, list(IKBKG_var_1_df, IKBKG_var_2_df, IKBKG_var_3_df, IKBKG_var_4_df, IKBKG_var_5_df, IKBKG_var_6_df, IKBKG_var_7_df, IKBKG_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('macaca IKBKG') + ylab('XCI skews') + ggtitle('macaca IKBKG')


```

Checking out the Snps associated with XCI skews in rat
```{r}

snp_skew_auc_with_meta_df %>% filter(label == 'Rat', skew_auc >= .75 )

```

```{r}
filter(rat_snp_stat_df, gene == 'RGD1566265' & num_samples_present >= 10) %>% arrange(desc(num_samples_present))

```

```{r}
sample_index = which(rat_skew_and_stats_df$num_good_snps_no_escape >= 10 & rat_skew_and_stats_df$autosome_imbalance == F)
filt_annot_rat_vcfs = annot_rat_vcfs[sample_index]
rat_XCI_skews = rat_skew_and_stats_df$est_skew_no_escape[sample_index]


#Wwc3
Wwc3_var_1_df =  get_var_data(filt_annot_rat_vcfs,'23697909', rat_XCI_skews)
Wwc3_var_2_df =  get_var_data(filt_annot_rat_vcfs,'23700971', rat_XCI_skews)
Wwc3_var_3_df =  get_var_data(filt_annot_rat_vcfs,'23691160', rat_XCI_skews)
Wwc3_var_4_df =  get_var_data(filt_annot_rat_vcfs,'23681620', rat_XCI_skews)
Wwc3_var_5_df =  get_var_data(filt_annot_rat_vcfs,'23681856', rat_XCI_skews)
Wwc3_var_6_df =  get_var_data(filt_annot_rat_vcfs,'23697347', rat_XCI_skews)
Wwc3_var_7_df =  get_var_data(filt_annot_rat_vcfs,'23699820', rat_XCI_skews)
Wwc3_var_8_df =  get_var_data(filt_annot_rat_vcfs,'23699653', rat_XCI_skews)

comb_test_df = Reduce(rbind, list(Wwc3_var_1_df, Wwc3_var_2_df, Wwc3_var_3_df, Wwc3_var_4_df, Wwc3_var_5_df, Wwc3_var_6_df, Wwc3_var_7_df, Wwc3_var_8_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('rat Wwc3') + ylab('XCI skews') + ggtitle('rat Wwc3')


#RGD1566265
RGD1566265_var_1_df =  get_var_data(filt_annot_rat_vcfs,'72860521', rat_XCI_skews)
RGD1566265_var_2_df =  get_var_data(filt_annot_rat_vcfs,'72860867', rat_XCI_skews)
RGD1566265_var_3_df =  get_var_data(filt_annot_rat_vcfs,'72860806', rat_XCI_skews)


comb_test_df = Reduce(rbind, list(RGD1566265_var_1_df, RGD1566265_var_2_df, RGD1566265_var_3_df))
ggplot(comb_test_df, aes(y = skews, x = variant, fill = data_label)) + geom_boxplot() + xlab('rat RGD1566265') + ylab('XCI skews') + ggtitle('rat RGD1566265')
...
escape_percent_vec = seq(.01, 1,.01)

total_genes = length(unique(human_snp_stat_df$gene[human_snp_stat_df$filter ==T & human_snp_stat_df$num_samples_present >= 10 ])) - 1
xci_associated_genes = 8
xci_asso_and_escape_genes = 4

enrich_pval_vec = vector(mode = 'numeric', length = length(escape_percent_vec))

for(i in 1:length(escape_percent_vec)){
  
  all_escape_genes = ceiling(total_genes * escape_percent_vec[i])
  enrich_pval_vec[i] = phyper(xci_asso_and_escape_genes-1, all_escape_genes, total_genes-all_escape_genes, xci_associated_genes,lower.tail= FALSE)

}
par(pty = 's')
plot(escape_percent_vec, enrich_pval_vec, xlab = '% of genes escaping XCI', ylab = 'p-value for enriched X-escapers (Fishers exact test)', main = '4 out of 8 genes')
abline(h = .05, col = 'red', lty = 'dashed')


xci_associated_genes = 17
xci_asso_and_escape_genes = 5
...
par(pty = 's')
plot(escape_percent_vec, enrich_pval_vec, xlab = '% of genes escaping XCI', ylab = 'p-value for enriched X-escapers (Fishers exact test)')
abline(h = .05, col = 'red', lty = 'dashed')
